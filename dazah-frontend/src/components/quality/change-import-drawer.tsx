...
export function ChangeImportDrawer({ isOpen, onClose, onSuccess }: ChangeImportDrawerProps) {
  ...
  async function handleDownloadTemplate() {
    setErrorMsg("")
    try {
      const res = await fetch("/api/v1/quality/changes/export")
      ...
      <div className="fixed top-0 right-0 h-full w-[480px] bg-white shadow-2xl z-50 transform transition-transform duration-300 translate-x-0">
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h2 className="text-lg font-semibold text-gray-900">导入变更数据</h2>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
            ...
            <div>
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 transition-colors">
                <input ref={fileInputRef} type="file" accept=".docx" onChange={handleFileChange} className="hidden" id="change-file-input" />
                <label htmlFor="change-file-input" className="cursor-pointer">
