...
export function DeviationImportDrawer({ isOpen, onClose, onSuccess }: DeviationImportDrawerProps) {
  ...
  async function handleDownloadTemplate() {
    const res = await fetch("/api/v1/quality/deviations/export")
    if (!res.ok) throw new Error("下载模板失败")
    const blob = await res.blob()
    const url = URL.createObjectURL(blob)
    const a = document.createElement("a")
    a.href = url
    a.download = "偏差登记表_模板.docx"
    a.click()
      ...
      <div className="fixed top-0 right-0 h-full w-[480px] bg-white shadow-2xl z-50 transform transition-transform duration-300 translate-x-0">
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h2 className="text-lg font-semibold text-gray-900">导入偏差数据</h2>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
            <div className="bg-blue-50 rounded-lg p-4">
              ...
              <div className="text-xs text-blue-600 mb-3">{deviationHeaders.join("、")}</div>
            ...
            <div>
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 transition-colors">
                <input ref={fileInputRef} type="file" accept=".docx" onChange={handleFileChange} className="hidden" id="deviation-file-input" />
                <label htmlFor="deviation-file-input" className="cursor-pointer">
