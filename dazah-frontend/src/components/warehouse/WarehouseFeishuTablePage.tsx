...
// 记录详情字段只读文本格式化：抽至 lib/format/warehouse（检验进度面板共用），
// 此处 re-export 维持既有导入方（测试等）不变
export { formatDetailDisplayValue } from '@/lib/format/warehouse'
...
// ── 检验进度周期（只读统计，详情弹窗展示） ────────────────────
// 状态色 / 时长与时间格式化 / 周期块渲染抽至共享模块（检验进度面板共用）
