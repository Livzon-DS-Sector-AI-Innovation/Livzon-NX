...
// ── 检验进度周期（只读统计，详情弹窗展示） ────────────────────
const INSPECTION_CYCLE_STATUS_COLORS: Record<string, string> = {
  completed: 'green',
  pending: 'gold',
  not_counted: 'default',
...
function formatInspectionHours(hours?: number | null): string {
  if (hours == null) {
    return '—'
  }
  if (hours < 48) {
...
function renderInspectionCycleBlock(cycle?: WarehouseInspectionCycle | null) {
  if (!cycle) {
    return null
  }
  return (
