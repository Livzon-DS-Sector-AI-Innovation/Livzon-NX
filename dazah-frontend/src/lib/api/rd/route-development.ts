...
export async function fetchRoutes(
  ...
  return apiFetchPaginated(
    `/api/v1/research/routes?${params.toString()}`
  )
}

export async function fetchRouteById(routeId: string): Promise<RouteDevelopment> {
  return apiFetch(`/api/v1/research/routes/${routeId}`)
}

export async function createRoute(data: RouteCreate): Promise<RouteDevelopment> {
