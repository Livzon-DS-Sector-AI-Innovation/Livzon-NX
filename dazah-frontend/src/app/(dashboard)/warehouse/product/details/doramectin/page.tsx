...
function createEmptyPageData(): WarehouseFeishuMaterialPageData {
  return {
    page_key: 'product-detail-doramectin',
    page_title: '多拉菌素库存明细',
    table_name: '多拉菌素库存明细',
    columns: [],
    rows: [],
...
export default async function WarehouseProductDoramectinPage({ searchParams }: WarehousePageProps) {
  let data = createEmptyPageData()

  try {
    data = await fetchWarehouseMaterialPage('product-detail-doramectin', {
      ...(await resolveWarehousePageQueryParams(searchParams))
    })
  } catch (error) {
    console.warn('多拉菌素库存明细页面初始数据加载失败，使用空数据降级:', error)
  }

  return <WarehouseFeishuTablePage data={data} pageKey='product-detail-doramectin' />
}
