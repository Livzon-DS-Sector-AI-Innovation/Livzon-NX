...
function createEmptyPageData(): WarehouseFeishuMaterialPageData {
  return {
    page_key: 'product-detail-lovastatin',
    page_title: '洛伐他汀库存明细',
    table_name: '洛伐他汀库存明细',
    columns: [],
    rows: [],
...
export default async function WarehouseProductLovastatinPage({ searchParams }: WarehousePageProps) {
  let data = createEmptyPageData()

  try {
    data = await fetchWarehouseMaterialPage('product-detail-lovastatin', {
      ...(await resolveWarehousePageQueryParams(searchParams))
    })
  } catch (error) {
    console.warn('洛伐他汀库存明细页面初始数据加载失败，使用空数据降级:', error)
  }

  return <WarehouseFeishuTablePage data={data} pageKey='product-detail-lovastatin' />
}
