...
function createEmptyPageData(): WarehouseFeishuMaterialPageData {
  return {
    page_key: 'product-detail-fenbendazole-powder',
    page_title: '芬苯达唑粉剂库存明细',
    table_name: '芬苯达唑粉剂库存明细',
    columns: [],
    rows: [],
    total: 0,
    page: 1,
...
export default async function WarehouseProductFenbendazolePowderPage({
  searchParams,
}: WarehousePageProps) {
  let data = createEmptyPageData()

  try {
    data = await fetchWarehouseMaterialPage('product-detail-fenbendazole-powder', {
      ...(await resolveWarehousePageQueryParams(searchParams))
    })
  } catch (error) {
    console.warn('芬苯达唑粉剂库存明细页面初始数据加载失败，使用空数据降级:', error)
  }

  return <WarehouseFeishuTablePage data={data} pageKey='product-detail-fenbendazole-powder' />
}
