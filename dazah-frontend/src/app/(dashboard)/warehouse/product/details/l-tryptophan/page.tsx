...
function createEmptyPageData(): WarehouseFeishuMaterialPageData {
  return {
    page_key: 'product-detail-l-tryptophan',
    page_title: 'L-色氨酸库存明细',
    table_name: 'L-色氨酸库存明细',
    columns: [],
    rows: [],
    total: 0,
    page: 1,
...
export default async function WarehouseProductLTryptophanPage({ searchParams }: WarehousePageProps) {
  ...
  try {
    ...
    console.warn('L-色氨酸库存明细页面初始数据加载失败，使用空数据降级:', error)
  }

  return <WarehouseFeishuTablePage data={data} pageKey='product-detail-l-tryptophan' />
}
