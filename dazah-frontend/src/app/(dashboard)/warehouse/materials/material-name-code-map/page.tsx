...
function createEmptyPageData(): WarehouseFeishuMaterialPageData {
  return {
    page_key: 'material-name-code-map',
    page_title: '物料名称及代码对应表',
    table_name: '物料名称及代码对应表',
    columns: [],
    rows: [],
    total: 0,
    page: 1,
...
export default async function WarehouseMaterialNameCodeMapPage({ searchParams }: WarehousePageProps) {
  ...
  try {
    ...
    console.warn('物料名称及代码对应表页面初始数据加载失败，使用空数据降级:', error)
  }

  return <WarehouseFeishuTablePage data={data} pageKey="material-name-code-map" />
}
