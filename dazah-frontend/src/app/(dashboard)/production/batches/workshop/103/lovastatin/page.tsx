...
const LINE_STORAGE_KEY = 'workshop_103_lovastatin_active_products'
...
export default function Workshop103LovastatinPage() {
  const { message, modal } = App.useApp()
  const { canOperate, canDelete, canExport } = useProductionPermissions(PRODUCTION_PAGE_KEYS.workshop103Lovastatin)
  ...
  // 月度计划值 — 按产品分别存储
  const planBatchesKey = `${activeKey}_plan_batches`
  const planYieldKey = `${activeKey}_plan_yield`
  const [planTotalBatches, setPlanTotalBatches] = useState<number>(() => {
    if (typeof window === 'undefined') return 0
    return Number(localStorage.getItem(`${activeKey}_plan_batches`) || 0)
  })
  const [planYield, setPlanYield] = useState<number>(() => {
    if (typeof window === 'undefined') return 0
    return Number(localStorage.getItem(`${activeKey}_plan_yield`) || 0)
  })
  ...
  const handleTabChange = (key: string) => {
    ...
    setPlanTotalBatches(Number(localStorage.getItem(`${key}_plan_batches`) || 0))
    setPlanYield(Number(localStorage.getItem(`${key}_plan_yield`) || 0))
  }
  ...
  return (
    <div className="p-6">
      ...
      {visibleProducts.length === 0 ? (
        <Card>
          ...
          <div style={{ textAlign: 'center', marginTop: 16 }}>
            {canOperate && <Button type="primary" icon={<ControlOutlined />} onClick={handleLineConfigOpen}>产线配置</Button>}
          </div>
        </Card>
      ) : (
        <>
      ...
      <Modal
        title="产线配置"
        open={lineConfigVisible && canOperate}
        onOk={canOperate ? handleLineConfigSave : undefined}
