...
export default function PhenylalaninePage() {
  ...
  const [planTotalBatches, setPlanTotalBatches] = useState<number>(() => {
    ...
    return Number(localStorage.getItem('phenylalanine_plan_batches') || 0)
  })
  const [planYield, setPlanYield] = useState<number>(() => {
    if (typeof window === 'undefined') return 0
    return Number(localStorage.getItem('phenylalanine_plan_yield') || 0)
  })
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await getFermentationRecords({ page: 1, page_size: 200, product_name: 'L-苯丙氨酸' })
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editing) {
        ...
        const res = await createFermentationRecord({ ...data, product_name: 'L-苯丙氨酸' } as FermentationCreate)
  ...
  const handlePlanSave = async () => {
    try {
      ...
      localStorage.setItem('phenylalanine_plan_batches', String(values.batches || 0))
      localStorage.setItem('phenylalanine_plan_yield', String(values.yield || 0))
