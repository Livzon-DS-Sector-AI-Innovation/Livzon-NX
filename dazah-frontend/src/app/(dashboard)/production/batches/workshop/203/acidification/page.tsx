...
export default function AcidificationPage() {
  ...
  const load = async (p = 1, ps = 20) => {
    setLoading(true)
    try {
      const r = await fetch(`/api/v1/production/fa/acidification/flat-list?page=${p}&page_size=${ps}${month > 0 ? `&month=${month}` : ''}`)
...
useEffect(() => {    fetch(`/api/v1/production/fa/monthly-averages?table=acidification_records`)      .then(r => r.json()).then(j => { if(j.code===200){setAvgData(j.data.data||[]);setAvgCols(j.data.columns||[])} }).catch(()=>{})  }, [])
    ...
    <div className="p-6">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <Space>
          ...
          <FATraceButton initialModule="acidification" pageKey="production:batches:workshop-203" />
        </Space>
      </div>
      {avgData.length > 0 && (
      ...
      <Card>
        <Table
          ...
          pagination={false}
          columns={[
            { title: '日期', dataIndex: '日期', width: 66, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
            { title: '批号', dataIndex: '批号', width: 84, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
            { title: '发酵液', children: [
              { title: '发酵液体积（kl)', dataIndex: '发酵液体积（kl)', width: 84 },
              { title: '发酵液含量（g/L）', dataIndex: '发酵液含量（g/L）', width: 84 },
              { title: '发酵液罐产（kg）', dataIndex: '发酵液罐产（kg）', width: 84, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
            ]},
            { title: '酸化液', children: [
              { title: '用酸量（95-98%浓硫酸）', dataIndex: '用酸量（95-98%浓硫酸）', width: 90, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
              { title: 'PH（酸化后）', dataIndex: 'PH（酸化后）', width: 60 },
              { title: '酸化液体积（kl)', dataIndex: '酸化液体积（kl)', width: 84, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
              { title: '理论酸化液含量（g/L）', dataIndex: '理论酸化液含量（g/L）', width: 90, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }), render: (v) => v != null ? Number(v).toFixed(2) : '' },
            ]},
            { title: '酸化滤液', children: [
              ...
              { title: '膜滤液产品总量（kg）', dataIndex: '膜滤液产品总量（kg）', width: 90, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
              ...
              { title: '本批低单位量（kg）', dataIndex: '本批低单位量（kg）', width: 84, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
              { title: '上批套用低单位量（kg）', dataIndex: '上批套用低单位量（kg）', width: 90, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
            ]},
            { title: '渣液', children: [
              { title: '批收率', dataIndex: '批收率', width: 54, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
              { title: '顶洗前体积（kl）', dataIndex: '顶洗前体积（kl）', width: 78, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
              { title: '尾液含量（g/L）', dataIndex: '尾液含量（g/L）', width: 72, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
              { title: '渣含量（g/L）', dataIndex: '渣含量（g/L）', width: 72, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
              { title: '体积（罐渣+膜渣（kl）', dataIndex: '体积（罐渣+膜渣（kl）', width: 90, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
              { title: '渣产品量（kg）', dataIndex: '渣产品量（kg）', width: 72, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
              { title: '渣损失率（渣苯丙量/罐产）', dataIndex: '渣损失率（渣苯丙量/罐产）', width: 90, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
            ]},
            { title: '标准数据', children: [
              { title: '渣体积/发酵液体积', dataIndex: '渣体积/发酵液体积', width: 90, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
              { title: '酸化液/发酵液体积', dataIndex: '酸化液/发酵液体积', width: 90, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
              { title: '滤液体积/发酵液体积', dataIndex: '滤液体积/发酵液体积', width: 90, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
              { title: '平衡率', dataIndex: '平衡率', width: 60, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
              { title: '消泡剂使用量（L）', dataIndex: '消泡剂使用量（L）', width: 90, onCell: (r) => ({ rowSpan: r._rowSpan ?? 1 }) },
            ]},
          ]}
        />
