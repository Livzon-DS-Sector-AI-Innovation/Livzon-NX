...
export default function IntermediatePage() {
  ...
  const load = async (p = 1, ps = 20) => {
    setLoading(true)
    try {
      const r = await fetch(`/api/v1/production/fa/intermediate/list?page=${p}&page_size=${ps}${month > 0 ? `&month=${month}` : ''}`)
...
useEffect(() => {    fetch(`/api/v1/production/fa/monthly-averages?table=intermediate_records`)      .then(r => r.json()).then(j => { if(j.code===200){setAvgData(j.data.data||[]);setAvgCols(j.data.columns||[])} }).catch(()=>{})  }, [])
  return (
    <div className="p-6">
