...
export default function MvrPage() {
  ...
  const load = async (p = 1, ps = 20) => {
    setLoading(true)
    try {
      const r = await fetch(`/api/v1/production/fa/mvr/list?page=${p}&page_size=${ps}${month > 0 ? `&month=${month}` : ''}`)
...
useEffect(() => {    fetch(`/api/v1/production/fa/monthly-averages?table=mvr_records`)      .then(r => r.json()).then(j => { if(j.code===200){setAvgData(j.data.data||[]);setAvgCols(j.data.columns||[])} }).catch(()=>{})  }, [])
    ...
    <div className="p-6">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <Space>
          ...
          <FATraceButton initialModule="mvr" pageKey="production:batches:workshop-203" />
        </Space>
      </div>
      {avgData.length > 0 && (
