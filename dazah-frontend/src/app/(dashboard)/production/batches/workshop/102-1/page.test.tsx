...
describe('FermentationPage', () => {
  ...
  it('opens the create and monthly-plan modals', async () => {
    container = document.createElement('div')
    document.body.append(container)
    root = createRoot(container)
    act(() => {
  ...
  function setInputValueUtl(el: HTMLInputElement, value: string) {
  ...
  it('calculates plan stats from saved localStorage values', async () => {
    window.localStorage.setItem('doramectin_plan_batches', '20')
    window.localStorage.setItem('doramectin_plan_yield', '2000')
    container = document.createElement('div')
    document.body.append(container)
    root = createRoot(container)
    act(() => {
    ...
    const text = (container.textContent || '') + (document.body.textContent || '')
    expect(text).toContain('计划总批次（批）')
    expect(text).toContain('计划产量（kg）')
    expect(text).toContain('罐产总计（kg）')
  })
  ...
  it('filters records by the batch number search input', async () => {
    ...
    if (input) {
      await act(async () => {
        setInputValueUtl(input, 'NOT-EXIST')
  ...
  it('saves the monthly plan values from the plan modal', async () => {
    ...
    if (batches) {
      await act(async () => {
        setInputValueUtl(batches, '20')
        await new Promise((r) => setTimeout(r, 30))
      })
    }
    if (yieldInput) {
      await act(async () => {
        setInputValueUtl(yieldInput, '1500')
        await new Promise((r) => setTimeout(r, 30))
      })
    }
    ...
    expect(window.localStorage.getItem('doramectin_plan_batches')).toBe('20')
    expect(window.localStorage.getItem('doramectin_plan_yield')).toBe('1500')
  })
