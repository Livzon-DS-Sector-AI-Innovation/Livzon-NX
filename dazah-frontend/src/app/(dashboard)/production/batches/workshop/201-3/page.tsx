'use client'
// 201三车间 DR（多拉菌素）— 首页（仪表盘 + 6宫格工段选择）— 按文档规格
...
import {ExperimentOutlined, FilterOutlined, BulbOutlined,
  ...
  ArrowRightOutlined, NodeIndexOutlined, ScheduleOutlined,} from '@ant-design/icons'
...
const STAGES = [
  { key: 'crude', label: '过滤萃取', desc: '过滤萃取', icon: <ExperimentOutlined style={{ fontSize: 40 }} />, color: '#52c41a', path: '/production/batches/workshop/201-3/crude-extraction' },
  { key: 'extraction', label: '层析及一次结晶岗位', desc: '层析→一次结晶', icon: <FilterOutlined style={{ fontSize: 40 }} />, color: '#1890ff', path: '/production/batches/workshop/201-3/extraction' },
  { key: 'refinement', label: '一次精制', desc: '一次精制', icon: <BulbOutlined style={{ fontSize: 40 }} />, color: '#13c2c2', path: '/production/batches/workshop/201-3/dr-refinement' },
  { key: 'blending', label: '二次精制', desc: '二次精制', icon: <CalculatorOutlined style={{ fontSize: 40 }} />, color: '#fa8c16', path: '/production/batches/workshop/201-3/blending' },
  { key: 'qc', label: '三次精制', desc: '三次精制', icon: <CheckCircleOutlined style={{ fontSize: 40 }} />, color: '#722ed1', path: '/production/batches/workshop/201-3/qc-inspection' },
  { key: 'ba', label: '四次精品', desc: '四次精品', icon: <DatabaseOutlined style={{ fontSize: 40 }} />, color: '#f5222d', path: '/production/batches/workshop/201-3/butyl-acetate' },
]

const STAGE_MAP: Record<string, (typeof STAGES)[number]> = {}
STAGES.forEach(s => { STAGE_MAP[s.key] = s })

export default function Workshop2013Page() {
  ...
  const fetchData = useCallback(async (monthStr: string) => {
    setLoading(true)
    try {
      const r = await fetch(`/api/v1/production/dr/dashboard/summary?month=${monthStr}`)
  ...
  return (
    <div className="p-6">
      <div className="mb-4">
        ...
        <Text type="secondary">201三车间 — DR（多拉菌素）粗提 → 提取 → 二次精制 → 混粉计算 → QC检验 → 入库</Text>
      </div>
      ...
      <Space className="mb-4" align="center" style={{ width: '100%', justifyContent: 'space-between' }}>
        ...
        <Space size={8}>
          <SyncSettingsButton productName="多拉菌素" syncTarget="dr_ledger" pageKey={PRODUCTION_PAGE_KEYS.workshop2013} />
          <Button type="primary" size="large" icon={<ScheduleOutlined />} onClick={() => router.push('/production/batches/workshop/201-3/scheduling')}>
            排产
          </Button>
          <Button size="large" icon={<NodeIndexOutlined />} onClick={() => router.push('/production/batches/workshop/201-3/traceability')}>
            全链路追溯
          </Button>
        </Space>
      </Space>
