...
import DrThirdRefinementPage from './page'
...
function fetchMock(url: string): Promise<Response> {
  if (url.includes('/dr/records?')) {
    return Promise.resolve(jsonResponse({ code: 200, message: 'success', data: { items: [{ id: 't1' }], total: 1 } }))
  }
...
describe('DrThirdRefinementPage (201-3/qc-inspection)', () => {
  ...
  it('renders the third refinement page with table and stage nav', async () => {
    vi.stubGlobal('fetch', vi.fn(fetchMock))
    act(() => {
      root.render(<App><DrThirdRefinementPage /></App>)
    })
    await act(async () => {
      await new Promise((r) => setTimeout(r, 80))
    })
    const text = container.textContent || ''
    expect(text).toContain('三次精制')
  ...
  it('shows an error when records fail to load', async () => {
    ...
    expect(container.textContent || '').toContain('三次精制')
  })
})