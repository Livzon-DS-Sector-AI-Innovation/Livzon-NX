'use client'
// DR 201三车间 — 层析及一次结晶岗位台账

import { useState, useEffect, useCallback, useMemo } from 'react'
import { useRouter } from 'next/navigation'
import { Card, Button, Space, Spin, Typography, App, Select } from 'antd'
import { ArrowLeftOutlined } from '@ant-design/icons'
import DRChromatographyTable from '@/components/production/DRChromatographyTable'
...
const STAGES = [
  ...
  { key: 'chromatography', label: '层析及一次结晶岗位', path: '/production/batches/workshop/201-3/extraction', active: true },
...
export default function DrExtractionPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      params.set('table', 'dr_chromatography_crystal')
      params.set('page', '1')
    ...
    <div className="p-6">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <Space size={8}>
          ...
          <DRTraceButton initialModule="chromatography" />
        </Space>
      </div>
      ...
      <Card>
        {loading ? (
          ...
          <DRChromatographyTable data={data} />
        )}
      </Card>
    </div>
  )
