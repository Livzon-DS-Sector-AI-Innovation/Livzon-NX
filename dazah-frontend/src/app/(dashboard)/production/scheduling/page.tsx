...
export default function SchedulingPage() {
  ...
  const { canDelete, canBulkImport, canExport } = useProductionPermissions(PRODUCTION_PAGE_KEYS.scheduling)
    ...
    },
    {
      ...
      render: (_: unknown, record: ScheduleExcelArchive) => (
        <Space size={4} onClick={(e) => e.stopPropagation()}>
          ...
          {canExport && <Button size="small" icon={<DownloadOutlined />} href={fileDownloadUrl(record.id)}>
            下载原件
          </Button>}
    ...
    <div className="p-6">
      ...
      {active && (
        <Card
          ...
          extra={
            <Space size={4}>
              {canExport && <Button size="small" icon={<DownloadOutlined />} href={fileDownloadUrl(active.id)}>
                下载原件
              </Button>}
