...
- 顶部模块导航应有一致的点击区域和间距，明确区分悬停、按下、当前模块和切换等待状态；键盘焦点可见，横向空间不足时保留可滚动的模块入口。
- 侧边栏多级菜单统一使用向右侧悬浮展开的子菜单，不在侧边栏内向下推开其他入口。鼠标悬停可展开，点击父级标题可切换展开状态；鼠标能从父级连续移入浮层，离开后可收起。
- 菜单展开不改变当前页面；选择叶子入口后关闭浮层并导航。带独立页面的父级标签点击仍导航到该页面，展开操作应由其余标题区域承担。当前页面入口保持高亮，但路由变化不自动弹出浮层。
- 多级浮层应按层级继续向右展开，并处理视口边缘、纵向溢出、键盘焦点与 Escape 关闭。窄屏或无悬停设备必须能通过点击操作，不得只依赖 hover。
- 同一层级的菜单项和父级标题保持一致的行高、左右对齐与垂直间距；悬停时使用明确但克制的背景和文字反馈，并与当前选中态、禁用态及键盘焦点区分。
- 菜单触发的页面加载反馈应放在不占用侧边栏布局的位置，不得让菜单入口在导航等待期间跳动；反馈需在路由完成或超时后消失，并保留可访问的状态提示。
