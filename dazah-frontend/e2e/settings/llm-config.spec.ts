...
test.describe('LLM 模型配置', () => {
  test('真实点击覆盖新建、探测、温度、激活、编辑和删除', async ({ page }) => {
    ...
    const activeSwitch = createDialog.getByRole('switch', { name: '激活状态' })
    const temperatureSwitch = createDialog.getByRole('switch', { name: '温度设置' })
    ...
    await expect(createDialog.getByRole('spinbutton', { name: '超时(秒)' })).toHaveValue('30')
    ...
    await expect(createdRow).toContainText('未激活')
    ...
    await createdRow.getByRole('button').nth(2).click()
    const editCreatedDialog = page.getByRole('dialog', { name: '编辑 LLM 配置' })
    const editCreatedSwitch = editCreatedDialog.getByRole('switch', { name: '激活状态' })
    await expect(editCreatedSwitch).toHaveAttribute('aria-checked', 'false')
    await editCreatedSwitch.click()
    await editCreatedDialog.locator('.ant-modal-footer .ant-btn-primary').click()
    ...
    const editActiveSwitch = editDialog.getByRole('switch', { name: '激活状态' })
    await expect(editActiveSwitch).toHaveAttribute('aria-checked', 'true')
    ...
    await expect(rowFor(page, '现有测试配置（已编辑）')).toContainText('未激活')

    const editedRow = rowFor(page, '现有测试配置（已编辑）')
    await editedRow.getByRole('button').nth(3).click()
