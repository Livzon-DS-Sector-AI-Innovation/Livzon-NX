[CmdletBinding()]
param(
  # Retained for compatibility; infrastructure uses prebuilt images.
  [switch]$NoBuild,
  ...
  # Remove the generated Next.js development cache before starting Frontend.
  # The default preserves it so Turbopack can reuse work across restarts.
...
$NativeToolsDir = Join-Path $Root ".dev-data\tools"
...
function Prepare-FrontendDevelopmentState {
  param(
    [Parameter(Mandatory)][string]$ProjectDirectory,
    [switch]$ResetCache
  )

  # Stop only Next processes belonging to this frontend. Keep the generated
  # route graph by default so Turbopack can reuse its filesystem cache.
...
$LarkCliManifestPath = Join-Path $HermesDir "lark-cli.json"
$LarkCliInstallerPath = Join-Path $HermesDir "scripts\install_pinned_lark_cli.py"
$LarkCliManifest = Get-Content -Raw -LiteralPath $LarkCliManifestPath | ConvertFrom-Json
$LarkCliVersion = [string]$LarkCliManifest.version
if ([string]::IsNullOrWhiteSpace($LarkCliVersion)) {
  throw "Hermes lark-cli manifest does not declare a version."
}
$LarkCliExecutableName = if ($env:OS -eq "Windows_NT") { "lark-cli.exe" } else { "lark-cli" }
$LarkCliPath = Join-Path $NativeToolsDir "lark-cli\$LarkCliVersion\$LarkCliExecutableName"
if (-not (Test-Path -LiteralPath $LarkCliPath)) {
  Write-Host "安装项目固定版本 lark-cli $LarkCliVersion..."
  try {
    Invoke-ExternalCommand `
      -FilePath $UvCommand `
      -Arguments @(
        "run", "--quiet", "--no-project", "python", $LarkCliInstallerPath,
        "--manifest", $LarkCliManifestPath,
        "--target", $LarkCliPath
      ) `
      -WorkingDirectory $HermesDir
  }
  catch {
    # Hermes can still serve non-Feishu-resource requests when the optional
    # CLI download is unavailable, so keep startup usable and report recovery.
    Write-Warning "lark-cli 安装失败；Hermes 仍可启动，但飞书资源 CLI 工具不可用。请检查网络后重新运行启动脚本。原因：$($_.Exception.Message)"
  }
}

if (Test-Path -LiteralPath $LarkCliPath) {
  Set-ProcessEnvValue -Name "LARK_CLI_PATH" -Value $LarkCliPath
}
else {
  # Clear a container-only value imported from .env.local so Hermes never
  # attempts to execute a Unix path from the native Windows process.
...
$ComposeUpArguments += @("db", "redis", "minio")

  Write-Host "启动 Docker 基础设施：PostgreSQL、Redis、MinIO..."
  ...
  Prepare-FrontendDevelopmentState `
    -ProjectDirectory $FrontendDir `
    -ResetCache:$ResetFrontendCache
  ...
  Write-Host "PostgreSQL、Redis、MinIO 容器会保留运行，便于下次快速启动。"
