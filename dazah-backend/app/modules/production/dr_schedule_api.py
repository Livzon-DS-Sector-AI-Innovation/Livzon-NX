...
class DumpPlanItem(BaseModel):
    ...
    task_status: str | None = None         # pending待接罐 / confirmed已确认 / delayed已填延期 / pending_approval待审批 / cancelled已取消
...
# 待资质表有数据后再打开。开启后：无该罐资质的人员确认 → 自动转 pending_approval 需班组长审批。
    ...
    for y, mo, d, batch, tank in plans:
        ...
        if row is None:
            await session.execute(text(
                "INSERT INTO production.receiving_task (batch_no, tank_no, plan_date, status) "
                "VALUES (:b, :t, :d, 'pending')"
            ), {"b": batch, "t": tank, "d": pd})
        elif row[1] == "pending":
            await session.execute(text(
                "UPDATE production.receiving_task SET tank_no = :t, plan_date = :d, updated_at = now() "
                "WHERE id = :id"
...
async def _receiving_task_map(session: AsyncSession) -> dict[str, dict]:
    ...
    rows = (await session.execute(text(
        ...
        "to_char(actual_time AT TIME ZONE 'Asia/Shanghai', 'YYYY-MM-DD HH24:MI:SS') AS actual_time, "
...
async def dr_dump_plans(
    ...
    upload_time = datetime.fromtimestamp(latest.stat().st_mtime).strftime("%Y-%m-%d %H:%M:%S")
    version = {"file": latest.name, "sheet": ws.title if ws else "", "upload_time": upload_time}
    if not plans:
        return success_response(data={"version": version, "items": [], "summary": {}})

    # DB 已有批号集合（实际投产/已接罐）
    db_rows = (await session.execute(text(
        "SELECT DISTINCT batch_no FROM production.dr_fermentation_batches WHERE is_deleted = false"
    ))).fetchall()
    ...
    for y, mo, d, batch, tank in plans:
        ...
        is_past = dd < today
        product_type = "中试批" if ("中试" in batch or batch.startswith("ZS-")) else "正式批"
        t = task_map.get(batch, {})
        items.append(DumpPlanItem(
            batch_no=batch, tank_no=tank, product_type=product_type,
            dump_date=dd.isoformat(), year=y, month=mo, day=d,
            in_db=in_db, is_past=is_past,
...
@router.post("/dr/schedule/upload", summary="DR 排产 Excel 上传更新（替代手动替换文件）")
async def dr_schedule_upload(
    ...
    if ext != ".xlsx":
        raise HTTPException(status_code=400, detail=f"仅支持 .xlsx 文件，收到 {ext or '(无扩展名)'}")
    ...
    ws = _pick_latest_sheet(wb)
    plans = _parse_dump_plans(ws)
    logger.info("[DR排产] 上传新排产 %s：sheet=%s 放罐 %d 条", saved_name, ws.title if ws else "-", len(plans))
...
async def dr_receiving_tasks(
    ...
    status: str = Query("", description="pending/confirmed/delayed/pending_approval/cancelled，空则全部"),
):
    ...
    rows = (await session.execute(text(
        ...
        "FROM production.receiving_task WHERE " + " AND ".join(where) + " ORDER BY plan_date"
    ), params)).fetchall()
    items = [{
        "batch_no": r[0],
        "tank_no": r[1],
...
async def confirm_receiving(
    ...
    if cur_status == "confirmed":
        raise HTTPException(status_code=400, detail=f"批号 {batch_no} 已确认接罐，不能重复操作")
    if cur_status == "delayed":
        raise HTTPException(status_code=400, detail=f"批号 {batch_no} 已填报延期，不能确认")
    ...
    if _QUALIFICATION_ENFORCED and current_user is not None:
        ...
        if emp_no:
            q = (await session.execute(text(
                ...
                "WHERE staff_no = :s AND tank_no = :t AND is_deleted = false LIMIT 1"
            ), {"s": emp_no, "t": actual_tank})).fetchone()
            needs_approval = bool(q is not None and not q[0])

    if needs_approval:
        await session.execute(text(
            "UPDATE production.receiving_task SET status = 'pending_approval', confirmed_by = :op, "
        ...
        return success_response(
            ...
            data={"task_status": "pending_approval", "confirmed_by": operator, "actual_tank_no": actual_tank},
        )

    now = datetime.now()
    await session.execute(text(
        "UPDATE production.receiving_task SET status = 'confirmed', actual_time = :at, confirmed_by = :op, "
        "actual_tank_no = :atk, note = :note, updated_at = now() WHERE id = :id"
    ), {"at": now, "op": operator, "atk": actual_tank, "note": body.note, "id": tid})
    await session.commit()
    logger.info("[接罐] %s 确认接罐，实测罐 %s，确认人 %s", batch_no, actual_tank, operator)
    return success_response(
...
async def delay_receiving(
    ...
    if cur_status not in _UNDONE_STATUS:
        raise HTTPException(status_code=400, detail=f"批号 {batch_no} 当前状态不允许填延期")

    operator = _operator_name(current_user, body.operator)
    await session.execute(text(
        "UPDATE production.receiving_task SET status = 'delayed', delay_reason = :r, note = :note, "
...
async def approve_receiving(
    ...
    if cur_status != "pending_approval":
        raise HTTPException(status_code=400, detail=f"批号 {batch_no} 当前状态不需要审批")

    operator = _operator_name(current_user, body.operator)
    if body.approve:
        now = datetime.now()
        await session.execute(text(
            "UPDATE production.receiving_task SET status = 'confirmed', actual_time = :at, "
            "approver = :op, approval_status = 'approved', updated_at = now() WHERE id = :id"
        ), {"at": now, "op": operator, "id": tid})
        await session.commit()
        logger.info("[接罐] %s 接罐已由 %s 批准", batch_no, operator)
        return success_response(
            message=f"{batch_no} 已批准接罐",
            data={"task_status": "confirmed", "approver": operator, "approval_status": "approved"},
        )
    ...
    return success_response(
        ...
        data={"task_status": "pending", "approver": operator, "approval_status": "rejected"},
    )
