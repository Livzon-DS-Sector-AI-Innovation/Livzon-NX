...
FIELD_MAPPING = {"日期":"clean_date","膜组件编号":"membrane_no","清洗剂类型":"cleaner_type","清洗剂浓度(%)":"cleaner_concentration","清洗温度(°C)":"clean_temp","清洗时间(min)":"clean_time","清洗压力(MPa)":"clean_pressure","水通量恢复率(%)":"flux_recovery","操作人":"operator","备注":"remarks"}
NUMBER_FIELDS = {"cleaner_concentration","clean_temp","clean_time","clean_pressure","flux_recovery"}
DATE_FIELDS = {"clean_date"}
...
async def sync_ceramic_clean(config: ProductionFeishuConfig, session: AsyncSession) -> dict:
    ...
    for item in items:
        ...
        if not mapped.get("membrane_no"): continue
