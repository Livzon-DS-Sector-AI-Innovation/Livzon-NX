...
FIELD_MAPPING = {"批次":"batch_no","二次浓缩结晶悬浮液进料总量(kg)":"feed_volume","离心转速(rpm)":"rotation_speed","分离时间(min)":"sep_duration","进料流速(L/h)":"feed_flow","精制湿晶体总量(kg)":"crystal_wet_weight","废母液总量(L)":"waste_liquor_volume","母液残留效价(U/mL)":"mother_liquor_titer","湿晶含水率(%)":"crystal_moisture","母液回收量(L)":"liquor_recovery","晶体收率(%)":"crystal_yield","备注":"remarks"}
NUMBER_FIELDS = {"rotation_speed","sep_duration","feed_flow","mother_liquor_titer","crystal_moisture","crystal_yield"}
DATE_FIELDS = {}
TABLE = "centrifuge2"
...
async def sync_centrifuge2(config: ProductionFeishuConfig, session: AsyncSession) -> dict:
    ...
    if unmatched:
        logger.warning("centrifuge2 同步警告：%d 个字段未匹配：%s", len(unmatched), unmatched)
