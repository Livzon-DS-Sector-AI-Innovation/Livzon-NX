...
FIELD_MAPPING = {
    ...
    "进料 pH 值": "feed_ph",
    "进料温度(℃)": "feed_temp",
    "进料效价(U/mL)": "feed_titer",
    "板框压力(MPa)": "filter_pressure",
    "进料流量(L/h)": "feed_flow",
    "过滤总时长(h)": "filter_duration",
    "滤布编号(-)": "cloth_no",
    "滤清液总量(L)": "filtrate_volume",
    "滤清液效价(U/mL)": "filtrate_titer",
    "滤饼（炭渣）湿重(kg)": "cake_wet_weight",
    "滤饼（炭渣）干重(kg)": "cake_dry_weight",
    "滤饼残留效价(U/g)": "cake_residue_titer",
    "板框管道残留量(L)": "pipe_residue",
    "滤饼含水率(%)": "cake_moisture",
    "过滤收率(%)": "filter_yield",
    "备注": "remarks",
}
...
async def sync_filter1(
    ...
    if unmatched:
        logger.warning(
            "filter1 同步警告：以下 %d 个映射字段在飞书中未找到数据：%s",
            len(unmatched),
            unmatched,
        )
