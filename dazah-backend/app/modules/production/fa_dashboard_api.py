...
async def get_fa_dashboard(
    ...
    async def _count_ferm(extra_where: str = "", params: dict | None = None) -> int:
        ...
        try:
            r = await session.execute(
                text(f"SELECT COUNT(1) FROM production.fa_fermentation_batches WHERE {where}"), p
            )
            return r.scalar() or 0
        except Exception:
    ...
    result["stages"] = {
        ...
        "decolor_centrifuge": await _count_month("fa_decolor_centrifuge_records", "日期"),
        "intermediate": 0,
    }
    ...
    try:
        r = await session.execute(
            text("""SELECT COALESCE(SUM("汇总总量_kg"), 0) FROM production.fa_fermentation_batches
                   WHERE "放罐日期" >= :start AND "放罐日期" < :end"""),
    ...
    try:
        r = await session.execute(
            text("""SELECT
                   COUNT(CASE WHEN "批收率" ~ '^[0-9.]+$' AND "批收率"::float >= 80 THEN 1 END) AS passed,
    ...
    for key, label, table, date_col in flow_stages:
        try:
            r = await session.execute(
                text(f'SELECT COUNT(1) FROM production.{table} WHERE "{date_col}" >= :start AND "{date_col}" < :end'),
                {"start": start_date, "end": end_date},
            )
    ...
    result["status_distribution"] = [
        {"status": "发酵完成", "count": result["stages"]["fermentation"], "color": "#1677ff"},
        {"status": "酸化完成", "count": result["stages"]["acidification"], "color": "#52c41a"},
        {"status": "脱色完成", "count": result["stages"]["decolor1"], "color": "#fa8c16"},
    ]
...
async def fa_yield_chain(
    ...
    # ── 2. 逐个批次查下游 ──
    batches = []
    stage_yields_all = {"fermentation": [], "acidification": [], "decolor1": [], "decolor_centrifuge": []}
    ...
    for fr in ferment_rows:
        ...
        item = {
            ...
            "fermentation_output": round(_to_float(fr.total_kg), 0) if fr.total_kg else 0,
            "fermentation_yield": round(_to_yield(fr.yield_rate), 1) if fr.yield_rate else None,
            "acid_batch": None, "acid_output": None, "acid_yield": None,
            "decolor1_batch": None, "centrifuge_batches": [],
            "centrifuge_avg_yield": None, "cumulative_yield": None,
        }
        ...
        # 酸化
        acid_rows = (await session.execute(text(
            """SELECT "批号", "膜滤液产品量（kg）" as mf_qty, "批收率" as yield_rate
               FROM production.fa_acidification_records
               WHERE "批号" = :bn AND "膜滤液产品量（kg）" IS NOT NULL LIMIT 1"""
        ), {"bn": fb})).fetchone()
        if acid_rows:
            item["acid_batch"] = acid_rows[0]
            item["acid_output"] = round(_to_float(acid_rows[1]), 0) if acid_rows[1] else 0
        ...
        cent_rows = (await session.execute(text(
        ...
        # 脱色一次
        if cent_rows:
            first_cent = cent_rows[0][0]  # FA-25316-1
            decolor_rows = (await session.execute(text(
        ...
        if item["acid_yield"] and item["centrifuge_avg_yield"]:
            item["cumulative_yield"] = round(item["acid_yield"] * item["centrifuge_avg_yield"] / 100, 1)
    ...
    stage_summary = []
    for sk, label in [("fermentation", "发酵放罐"), ("acidification", "酸化过滤"),
        ...
        if vals:
            ...
            stage_summary.append({
                "stage": sk, "label": label, "count": n,
                "avg_yield": _avg(vals),
                "min_yield": round(min(vals), 1),
                "max_yield": round(max(vals), 1),
                "q1": round(sorted_vals[n // 4], 1) if n >= 4 else sorted_vals[0],
                "median": round(sorted_vals[n // 2], 1) if n >= 2 else sorted_vals[0],
                "q3": round(sorted_vals[n * 3 // 4], 1) if n >= 4 else sorted_vals[-1],
            })
    ...
    # 找出最大损失阶段
    losses = [("酸化过滤", 100 - _avg(stage_yields_all.get("acidification", [100]))),
...
async def fa_golden_batches(
    ...
    ferment_sql = text("""
        ...
        ORDER BY fb."放罐日期" DESC
    """)
    ferment_rows = (await session.execute(ferment_sql, {"min_date": three_months_ago})).fetchall()
    ...
    fields = ["conductivity", "acid_adj", "filter_speed", "acid_used", "ph",
              "slag_loss", "carbon", "carbon_after", "centrifuge_avg", "centrifuge_std"]
    reference = {}
    for f in fields:
        label = {"conductivity": "电导(us/cm)", "acid_adj": "调酸量(L)", "filter_speed": "滤速(ml/10min)",
                 "acid_used": "用酸量(kg)", "ph": "pH(酸化后)", "slag_loss": "渣损失率(%)",
                 "carbon": "活性炭(kg)", "carbon_after": "碳后含量(g/L)",
                 "centrifuge_avg": "离心均值(%)", "centrifuge_std": "离心离散度"}[f]
...
_SUGGESTION_RULES = {
    "conductivity": {
        "high": {
            ...
            "remedy": "酸化时可适当增加调酸量和冲洗水用量，加大膜滤通量，减轻膜表面污染。",
            "impact": "调控得当可减少膜滤时间约5~10%，降低后续蒸发结垢概率。",
            "prevent": "关注菌种培养阶段的盐分控制，优化放罐时机，避免发酵后期电导急剧升高。",
        },
        "low": {
            "happened": "电导偏低，盐分少、提炼负担轻，但需确认是菌种代谢充分还是营养不足导致发酵不完整。",
            "remedy": "正常操作即可，可适当减少调酸量和冲洗水用量，降低辅料成本。",
            "impact": "提炼难度小，膜滤通量高，预期收率优于常规批次。",
            "prevent": "记录低电导批次的放罐参数（体积、含量、菌体浓度），建立优质批次档案作为参照。",
        },
    },
    "acid_adj": {
        "low": {
        ...
        "high": {
            "happened": "调酸量偏高，pH可能过低，过度酸化会增加硫酸消耗成本，并可能导致蛋白变性和产品降解。",
            "remedy": "检查酸化后pH值，若低于目标范围应减少下一批用酸量。当前批次加大膜滤顶洗量减少酸残留。",
            "impact": "调酸量回归标准后每批可节省硫酸约5~10%。",
            "prevent": "按发酵液体积和电导水平计算理论用酸量，建立分档对照表，避免凭经验过量加酸。",
        },
    },
    "slag_loss": {
        "high": {
            "happened": "渣损失率偏高，部分苯丙氨酸随渣饼流失，降低了酸化工段收率。",
            "remedy": "后端离心工段提高操作精度，减少甩料损失。每份离心独立计量，发现收率低立即调整。",
            "impact": "离心精度提升可追回约1~3%的渣损失。",
            "prevent": "完善膜滤CIP清洗程序，确保每次卸渣前充分顶洗；定期检查滤布完好性。",
        }
    },
    "filter_speed": {
        "low": {
            "happened": "酸化液滤速偏慢，发酵液粘度或菌体量偏高，膜污染风险增大。",
            "remedy": "加大膜滤顶洗频率和冲洗水量；必要时适当提高操作温度降低料液粘度。",
    ...
    "carbon_after": {
        "low": {
            ...
            "prevent": "检查活性炭品牌和批次质量，优化炭量配比；确保脱色温度和时间参数一致。",
        },
        "high": {
            "happened": "碳后含量异常偏高，可能是活性炭用量不足、脱色时间不够，或碳前含量测量有误（碳后不应高于碳前）。",
            "remedy": "确认碳前和碳后取样是否有误。若数据属实，本批增加活性炭用量或延长脱色时间，下批按此调整。",
            "impact": "碳后含量回归正常后离心收率更稳定，成品质量一致性提升。",
            "prevent": "每批记录活性炭用量、脱色温度和时长，定期校验碳前/碳后含量检测方法的一致性。",
        },
    },
    "centrifuge_std": {
        "high": {
            "happened": "8份离心收率差异过大（离散度高），分批和母液回用操作不一致，已造成不可逆损失。",
            "remedy": "混粉时将收率<95%的份与>100%的份搭配混合，利用高收率份拉高均值。",
            "impact": "高/低搭配混粉可提升最终收率约2~3个百分点。",
            "prevent": "规范离心分批作业：统一进料体积（目标95~100kl）、统一甩料时长、母液回用限定本批内。",
        }
    },
    "centrifuge_avg": {
        "low": {
            "happened": "离心平均收率低于黄金水平，整体提炼效率有优化空间。",
            "remedy": "检查各份离心的进料体积和甩料时长是否一致，优化炭后料液分配均匀性。",
            "impact": "操作标准化后离心均值可提升3~5个百分点。",
            "prevent": "建立离心操作SOP，每批记录各份进料体积/甩料时长/收率，定期分析趋势。",
        },
        "high": {
            "happened": "离心平均收率超过100%，说明母液回用混入了上层残液或上一批的残留，数值虚高不代表真实收率。",
            "remedy": "排查母液回用管道是否有串料，确认离心机清洗是否彻底。混粉时注意搭配低收率份拉回正常范围。",
            "impact": "规范母液回用流程后收率数据更真实，便于准确评估工艺水平。",
            "prevent": "母液回用严格限定本批内，建立回用比例上限（建议不超过进料体积的5%），每批记录回用量。",
        },
    },
    "ph": {
        "high": {
            "happened": "酸化后pH偏高，酸化不充分，会影响膜滤效率和蛋白去除效果。",
            "remedy": "本批补充酸量至pH达标后再进行膜滤。若已进入膜滤阶段，加大顶洗水量减少残留。",
            "impact": "pH调至标准范围后膜滤通量可提升10~15%，渣损失率降低。",
            "prevent": "建立pH-用酸量对照曲线，每批根据发酵液体积和电导预估用酸量，酸化后取样确认pH再放行。",
        },
        "low": {
            "happened": "酸化后pH偏低，用酸过量，可能造成蛋白过度变性、产品降解，且浪费硫酸。",
            "remedy": "当前批次减少下一轮用酸量。已酸化的料液在膜滤时加大顶洗量，减少酸残留对下游的影响。",
            "impact": "用酸量优化后每批节省硫酸成本，且产品色泽和纯度更稳定。",
            "prevent": "严格控制酸化终点pH，每批取样确认，建立不同电导水平对应的用酸量分档表。",
        },
    },
    "acid_amount": {
        "high": {
            "happened": "硫酸用量偏高，和调酸量偏高通常是同一原因，酸化过度会增加成本和下游处理难度。",
            "remedy": "核对当前批次pH值，若pH已达标则下批降低酸量；若pH仍高则检查酸浓度是否有误。",
            "impact": "酸量优化后每批节省硫酸约5~10%，同时减少膜滤冲洗水用量。",
            "prevent": "硫酸用量与发酵液体积、电导水平挂钩，建立标准化配比表，避免按经验过量投加。",
        },
        "low": {
            "happened": "硫酸用量偏低，酸化可能不充分，蛋白去除不完全，膜滤时容易堵膜。",
            "remedy": "确认酸化后pH是否达标，若不达标补充酸量。膜滤时加大顶洗频率弥补酸化不足。",
            "impact": "酸化充分后膜滤收率可提升1~2%，膜清洗周期延长。",
            "prevent": "建立最小用酸量标准，按发酵液体积和电导设定安全下限，酸化后必须取样确认pH。",
        },
    },
    "carbon": {
        "high": {
            "happened": "活性炭用量偏高，过度脱色可能吸附有效成分，造成含量损失，且增加固废处理成本。",
            "remedy": "当前批次已无法挽回，下批降低炭量至黄金参考范围。关注离心收率是否偏低。",
            "impact": "炭量回归标准后碳后含量提升，离心收率可提高1~2个百分点，废炭处理量减少。",
            "prevent": "活性炭用量与料液体积、色素水平挂钩，建立分档配比表。定期评估不同品牌活性炭的吸附选择性。",
        },
        "low": {
            "happened": "活性炭用量偏低，脱色不充分，可能导致成品色泽和透光率不达标。",
            "remedy": "本批若尚未完成脱色可补充活性炭延长脱色时间。若已完成，关注成品色泽检测结果。",
            "impact": "炭量达标后成品透光率和色泽稳定性提升，减少因外观不合格导致的返工。",
            "prevent": "设定活性炭用量下限，每批根据料液色泽目测判断，异常深色料液自动上浮炭量。",
        },
    },
}
...
async def fa_batch_params(
    ...
    params = {
        "conductivity": {"value": round(_to_float(fb.conductivity), 0) if fb.conductivity else None,
                         "label": "电导(us/cm)", "stage": "发酵放罐"},
        "acid_adj": {"value": round(_to_float(fb.acid_adj), 1) if fb.acid_adj else None,
                     "label": "调酸量(L)", "stage": "发酵放罐"},
        "filter_speed": {"value": round(_to_float(fb.filter_speed), 1) if fb.filter_speed else None,
                         "label": "滤速(ml/10min)", "stage": "发酵放罐"},
        "acid_used": {"value": round(_to_float(ar[0]), 0) if ar and ar[0] else None,
                      "label": "用酸量(kg)", "stage": "酸化过滤"},
        "ph": {"value": round(_to_float(ar[1]), 2) if ar and ar[1] else None,
               "label": "pH(酸化后)", "stage": "酸化过滤"},
        "slag_loss": {"value": round(_to_float(ar[2]), 2) if ar and ar[2] else None,
                      "label": "渣损失率(%)", "stage": "酸化过滤"},
        "carbon": {"value": round(_to_float(dr[0]), 0) if dr and dr[0] else None,
                   "label": "活性炭(kg)", "stage": "一次脱色"},
        "carbon_after": {"value": round(_to_float(dr[1]), 2) if dr and dr[1] else None,
                         "label": "碳后含量(g/L)", "stage": "一次脱色"},
    }
    ...
    if cent_yields:
        ...
        params["centrifuge_avg"] = {"value": round(avg_cy, 1), "label": "离心均值(%)", "stage": "脱色离心"}
        params["centrifuge_std"] = {"value": round(std_cy, 2), "label": "离心离散度", "stage": "脱色离心"}

    # ── 3. 获取黄金参考（内联计算，复用 golden-batches 逻辑）──
    ferment_all = (await session.execute(text(
        """SELECT fb."发酵罐号" as batch_no, fb."汇总总量_kg" as total_kg,
           fb."电导_uscm" as conductivity, fb."调酸量_L" as acid_adj,
           fb."酸化液滤速_ml10min" as filter_speed
           FROM production.fa_fermentation_batches fb
           ...
           ORDER BY fb."放罐日期" DESC"""
    ), {"min_date": three_months_ago})).fetchall()

    golden_entries = []
    for fr in ferment_all:
        entry = {"batch_no": fr.batch_no}
        ar2 = (await session.execute(text(
            """SELECT "用酸量（95-98%浓硫酸）" as acid, "PH（酸化后）" as ph,
               "渣损失率（渣苯丙量/罐产）" as slag, "批收率" as yield_rate
               FROM production.fa_acidification_records WHERE "批号" = :bn LIMIT 1"""
        ), {"bn": fr.batch_no})).fetchone()
        if not ar2: continue
        ...
        entry["conductivity"] = round(_to_float(fr.conductivity), 0) if fr.conductivity else 0
        entry["acid_adj"] = round(_to_float(fr.acid_adj), 1) if fr.acid_adj else 0
        entry["filter_speed"] = round(_to_float(fr.filter_speed), 1) if fr.filter_speed else 0
        entry["total_output"] = round(_to_float(fr.total_kg), 0)
        core2 = fr.batch_no.replace("FA-EX", "")
        dr2 = (await session.execute(text(
            """SELECT "活性炭添加量(kg)" as carbon, "碳后含量(g/L)" as after_c
               FROM production.fa_decolor1_records WHERE "批号" LIKE '%' || :core || '%' LIMIT 1"""
        ), {"core": core2})).fetchone()
        if dr2:
            entry["carbon"] = round(_to_float(dr2[0]), 0) if dr2[0] else 0
            entry["carbon_after"] = round(_to_float(dr2[1]), 2) if dr2[1] else 0
        crs2 = (await session.execute(text(
            """SELECT "收率" FROM production.fa_decolor_centrifuge_records WHERE "批号" LIKE '%' || :core || '%'"""
        ), {"core": core2})).fetchall()
        ...
        if len(cent_ys) >= 3:
            ...
            entry["centrifuge_std"] = round((sum((y-c_avg)**2 for y in cent_ys)/len(cent_ys))**0.5, 2)
    ...
    for f in fields:
        ...
        if vals:
            golden_ref[f] = {"avg": round(sum(vals)/len(vals), 2), "min": min(vals), "max": max(vals)}

    # ── 4. 计算偏离 + 生成建议 ──
    stages: dict[str, list[dict]] = {"发酵放罐": [], "酸化过滤": [], "一次脱色": [], "脱色离心": []}
    deviations = {}

    key_order = ["conductivity", "acid_adj", "filter_speed", "acid_used", "ph", "slag_loss",
                 "carbon", "carbon_after", "centrifuge_avg", "centrifuge_std"]
    for key in key_order:
        ...
        # 偏离方向
        direction = "high" if dev > _DEVIATION_THRESHOLD else "low" if dev < -_DEVIATION_THRESHOLD else "normal"
        p["direction"] = direction
        p["severity"] = "normal" if abs(dev) < 10 else "warn" if abs(dev) < 20 else "danger"
    ...
    try:
        save_data = AiAnalysis(
            batch_no=batch_no, stage="batch_compare", include_siblings=False,
            trace_snapshot={"params": {k: v["value"] for k, v in params.items() if v.get("value") is not None}},
            dist_snapshot={"golden_ref": {k: v["golden_avg"] for k, v in params.items() if v.get("golden_avg") is not None}},
            anomalies=[{"key": k, "deviation": deviations[k], "direction": params[k].get("direction")}
                       for k in deviations if abs(deviations[k]) >= _DEVIATION_THRESHOLD],
            llm_response="", summary=f"批次 {batch_no} vs 黄金批次对比",
            causes=[], suggestions=[],
            severity="low", model_used="", reference_cases=[],
            created_by="批次对比", session_id="",
            message_seq=0, role="batch_compare")
        session.add(save_data); await session.commit()
