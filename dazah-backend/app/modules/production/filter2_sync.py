...
FIELD_MAPPING = {
    "批次": "batch_no",
    "重结晶脱色悬浮液总进料量(kg)": "feed_volume",
    "过滤压力(MPa)": "filter_pressure",
    "过滤时长(min)": "filter_duration",
    "滤布型号": "cloth_type",
    "晶体滤饼湿重(kg)": "cake_wet_weight",
    "晶体滤饼干重(kg)": "cake_dry_weight",
    "晶体纯度(%)": "crystal_purity",
    "晶体效价(U/g)": "crystal_titer",
    "二次母液滤液总量(L)": "filtrate_volume",
    "母液残留效价(U/mL)": "mother_liquor_titer",
    "滤饼洗涤用水量(L)": "wash_water",
    "洗涤后母液合并量(L)": "combined_liquor",
    "洗涤损失(%)": "wash_loss",
    "备注": "remarks",
}
NUMBER_FIELDS = {
...
async def sync_filter2(
    ...
    if unmatched:
        logger.warning(
            "filter2 同步警告：%d 个字段未匹配：%s", len(unmatched), unmatched
        )
    return {
        "created": created,
        "updated": updated,
