...
class FermentationRecord(BaseModel):
    ...
    fermenter: Mapped[str] = mapped_column(String(64), nullable=False, comment="发酵罐")
    entry_date: Mapped[date] = mapped_column(Date, nullable=False, comment="进罐日期")
    discharge_date: Mapped[date | None] = mapped_column(
        Date, nullable=True, comment="放罐日期"
    )
    cycle_1: Mapped[float | None] = mapped_column(Float, nullable=True, comment="周期1")
    cycle_2: Mapped[float | None] = mapped_column(Float, nullable=True, comment="周期2")
    cycle_3: Mapped[float | None] = mapped_column(Float, nullable=True, comment="周期3")
    cycle_4: Mapped[float | None] = mapped_column(Float, nullable=True, comment="周期4")
    cycle_5: Mapped[float | None] = mapped_column(Float, nullable=True, comment="周期5")
    cycle_6: Mapped[float | None] = mapped_column(Float, nullable=True, comment="周期6")
