...
class BlendingRecord(BaseModel):
    ...
    # ── 杂质计算结果（加权平均）──
    rrt_053: Mapped[float | None] = mapped_column(
        Float, nullable=True, comment="RRT=0.53"
    )
    rrt_0755: Mapped[float | None] = mapped_column(
        Float, nullable=True, comment="RRT=0.755"
    )
    rrt_094_096: Mapped[float | None] = mapped_column(
        Float, nullable=True, comment="RRT=0.94-0.96"
    )
    rrt_103_106: Mapped[float | None] = mapped_column(
        Float, nullable=True, comment="RRT=1.03-1.06"
    )
    rrt_201: Mapped[float | None] = mapped_column(
        Float, nullable=True, comment="RRT=2.01"
    )
    total_impurity: Mapped[float | None] = mapped_column(
