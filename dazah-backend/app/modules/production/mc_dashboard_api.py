...
async def get_mc_dashboard(
    ...
    async def _count_blending(
        ...
        try:
            r = await session.execute(
                text(f"SELECT COUNT(1) FROM production.blending_records WHERE {where}"),
                p,
            )
            return r.scalar() or 0
        except Exception:
    ...
    try:
        r = await session.execute(
            text("""SELECT COALESCE(SUM(total_weight), 0) FROM
        production.blending_records
    ...
    try:
        r = await session.execute(
            text("""SELECT COALESCE(AVG(yield_rate), 0) FROM
        production.extraction_records
                   WHERE workshop = :ws AND is_deleted = false AND yield_rate IS NOT
        NULL
    ...
    flow = []
    stage_defs_date = [
        ("crude", "粗提", "refining_batches", "produce_date", "workshop = :ws"),
        (
            "extraction",
            "提取",
            "extraction_records",
        ...
        ("qc", "QC检验", "qc_inspections", "input_date", "status < 2"),
    ]
    ...
    try:
        ...
        flow.append(
            {"key": "blending", "label": "混粉计算", "in_progress": r.scalar() or 0}
        )
    except Exception:
        ...
        for label, (col, limit) in rrt_limits.items():
            r = await session.execute(
                text(f"""SELECT COUNT(1) AS total,
                   COUNT(CASE WHEN {col} IS NOT NULL AND {col} <= :limit THEN 1 END) AS
        passed
                   FROM production.blending_records
                   WHERE workshop = :ws AND is_deleted = false
                   AND SUBSTRING(batch_no FROM 4 FOR 2) = :yy
        ...
        try:
            ...
            result["ba_batches"] = (
                await session.execute(
                    text(
                        "SELECT COUNT(*) FROM production.butyl_acetate_records WHERE is_deleted = false AND is_check = true"  # noqa: E501
                    )
                )
            ).scalar() or 0
            ...
            r2 = await session.execute(
                text("""SELECT COALESCE(SUM(consumption), 0) FROM
        production.butyl_acetate_records
                       WHERE is_deleted = false AND is_check = false AND is_inbound =
        false
    ...
    try:
        r = await session.execute(
            text("""SELECT
                   COUNT(1) FILTER (
                     WHERE NOT EXISTS (SELECT 1 FROM production.blending_inputs WHERE
        blend_batch = b.batch_no)
                   ) AS pending,
                   COUNT(1) FILTER (
                     WHERE EXISTS (SELECT 1 FROM production.blending_inputs WHERE
        blend_batch = b.batch_no)
                   ) AS calculated,
                   COUNT(1) FILTER (
                     WHERE EXISTS (
                       ...
                       AND EXISTS (SELECT 1 FROM production.qc_inspection_inputs i
        WHERE i.qc_batch = q.batch_no)
                     )
                   ) AS completed
