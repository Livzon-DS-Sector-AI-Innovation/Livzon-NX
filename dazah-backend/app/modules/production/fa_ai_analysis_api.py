"""FA 苯丙氨酸 - AI 批次分析 API"""
import json, logging, re, uuid as _uuid
...
async def _get_trace_data(session, batch_no, stage):
    ...
    # 发酵数据
    ferment = (await session.execute(text(
        """SELECT fb."发酵罐号", fb."放罐日期"::text, fb."汇总总量_kg", fb."电导_uscm",
           fb."调酸量_L", fb."酸化液滤速_ml10min", fb."发酵液湿固",
           STRING_AGG(fsb."发酵批号" || '(' || COALESCE(fsb."放罐体积_kl"::text,'?') || 'kl/' || COALESCE(fsb."放罐含量_gL"::text,'?') || 'gL/' || COALESCE(fsb."批总量_kg"::text,'?') || 'kg)', ', ')
           as subs
        FROM production.fa_fermentation_batches fb
        LEFT JOIN production.fa_fermentation_sub_batches fsb ON fsb."父发酵罐号" = fb."发酵罐号"
        WHERE fb."发酵罐号" = :bn GROUP BY fb."发酵罐号", fb."放罐日期", fb."汇总总量_kg", fb."电导_uscm", fb."调酸量_L", fb."酸化液滤速_ml10min", fb."发酵液湿固" """
    ), {"bn": batch_no})).fetchone()
    if ferment:
        data_sections.append(f"发酵放罐: {batch_no}\n"
            f"  放罐日期: {ferment[1]}\n"
            f"  汇总总量: {ferment[2]}kg\n"
    ...
    if acid_rows:
        ...
        for ar in acid_rows:
            if ar.mf_qty and float(ar.mf_qty) > 0:
                sections.append(f"  膜滤: {ar.mf_vol}kl × {ar.mf_content}g/L = {ar.mf_qty}kg")
        data_sections.append(f"酸化过滤:\n"
    ...
    if cent_rows:
        ...
        data_sections.append(f"脱色离心 ({len(cent_rows)}份):\n" + "\n".join(cent_parts))

    # 统计数据
    stats = []

    cond = (await session.execute(text(
        """SELECT ROUND(AVG("电导_uscm")::numeric,0), ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY "电导_uscm")::numeric,0),
    ...
    if cond:
        stats.append(f"电导全量统计(us/cm): 均值{cond[0]} | 中位{cond[1]} | Q3={cond[2]} | 范围 {cond[3]}~{cond[4]}")

    yr_stats = (await session.execute(text(
        """SELECT ROUND(AVG("收率")::numeric*100, 1), ROUND(MIN("收率")::numeric*100, 1), ROUND(MAX("收率")::numeric*100, 1)
        FROM production.fa_decolor_centrifuge_records WHERE "收率" IS NOT NULL"""
    ))).fetchone()
    if yr_stats:
        stats.append(f"离心收率全量统计: 均值{yr_stats[0]}% | 范围 {yr_stats[1]}%~{yr_stats[2]}%")

    # 渣损失率统计
    slag = (await session.execute(text(
        """SELECT ROUND(AVG(CAST(REPLACE(REPLACE("渣损失率（渣苯丙量/罐产）", '%', ''), ',', '') AS numeric)), 1)
        FROM production.fa_acidification_records WHERE "渣损失率（渣苯丙量/罐产）" IS NOT NULL AND "渣损失率（渣苯丙量/罐产）" != ''"""
    ))).fetchone()
    if slag and slag[0]:
...
async def fa_ai_analysis(
    ...
    # 2. 构建 prompt
    prompt = f"""你是丽珠制药厂 203 车间 FA（L-苯丙氨酸）提炼工艺专家。请分析以下批次的生产数据，给出专业判断。
...
=== 工艺背景 ===
L-苯丙氨酸提炼流程：发酵放罐 → 酸化调酸 → 陶瓷膜过滤(3轮) → 一次脱色(活性炭) → 脱色离心(分8份各自离心，母液内部回用)
...
{{
  ...
  "severity": "low/medium/high"
    ...
    try:
        ...
        client = AsyncOpenAI(base_url=base_url, api_key=cfg.api_key or "no-key", timeout=120)
        ...
        try:
            await session.execute(text(
                "INSERT INTO production.ai_analysis (batch_no, stage, llm_response, summary, causes, suggestions, severity, model_used, created_by, message_seq, role) "
                "VALUES (:bn, :st, '', :summary, '[]'::jsonb, '[]'::jsonb, :severity, :model, 'AI自动分析', 0, 'assistant')"
            ), {"bn": batch_no, "st": stage, "summary": parsed.get("summary", "")[:500],
                "severity": parsed.get("severity", "low"), "model": model})
...
FA_ANALYSIS_PROMPT = """你是丽珠制药厂 203 车间 FA（L-苯丙氨酸）提炼工艺专家。请分析以下批次的生产数据，给出专业判断。
...
@router.get("/fa/lineage/ai-analysis-stream", summary="FA 批次 AI 分析（SSE 流式，展示思考过程）")
async def fa_ai_analysis_stream(
    ...
    if stage not in FA_STAGE_LABELS:
        raise HTTPException(400, f"无效工段: {stage}，可选: {list(FA_STAGE_LABELS.keys())}")
    ...
    async def generate():
        ...
        yield evt("step", {"step": "trace", "done": True,
        ...
        anomaly_count = 0
        if stats_data:
            yield evt("step", {"step": "stats", "done": True,
        ...
        try:
            # 离心收率异常检测
            cent_rows_raw = (await session.execute(text(
                """SELECT "批号", "收率" FROM production.fa_decolor_centrifuge_records
                   WHERE "批号" LIKE '%' || :core || '%' ORDER BY "批号" LIMIT 20"""
            ), {"core": batch_no.replace("FA-EX", "")})).fetchall()
            for cr in cent_rows_raw:
                yr = float(cr[1]) if cr[1] else 0
                if yr < 0.85:
                    anomalies.append({"stage": "decolor_centrifuge", "batch_no": cr[0],
                        "metric": "yield_rate", "value": round(yr*100, 1), "benchmark": 85,
                        "severity": "high" if yr < 0.75 else "medium",
                        "detail": f"离心收率{round(yr*100,1)}%，低于85%警戒线"})
                elif yr > 1.05:
                    anomalies.append({"stage": "decolor_centrifuge", "batch_no": cr[0],
                        "metric": "yield_rate", "value": round(yr*100, 1), "benchmark": 105,
                        "severity": "low",
                        "detail": f"离心收率{round(yr*100,1)}%>100%，可能母液回用混入残液"})
            ...
            if cond_rows and cond_rows[0]:
                ...
                if cond_val > 3000:
                    anomalies.append({"stage": "fermentation", "batch_no": batch_no,
                        "metric": "conductivity", "value": round(cond_val, 0), "benchmark": 3000,
                        "severity": "high" if cond_val > 5000 else "medium",
                        "detail": f"电导{cond_val}us/cm，高于3000，提炼难度大"})
                elif cond_val > 2000:
                    anomalies.append({"stage": "fermentation", "batch_no": batch_no,
                        "metric": "conductivity", "value": round(cond_val, 0), "benchmark": 2000,
                        "severity": "low",
                        "detail": f"电导{cond_val}us/cm，略高于车间均值"})

            # 渣损失率检测
            slag_rows = (await session.execute(text(
                """SELECT CAST(REPLACE(REPLACE("渣损失率（渣苯丙量/罐产）", '%', ''), ',', '') AS numeric)
                   FROM production.fa_acidification_records WHERE "批号" = :bn"""
            ), {"bn": batch_no})).fetchone()
            if slag_rows and slag_rows[0]:
                slag_val = float(slag_rows[0])
                if slag_val > 10:
                    anomalies.append({"stage": "acidification", "batch_no": batch_no,
                        "metric": "slag_loss", "value": round(slag_val, 1), "benchmark": 10,
                        "severity": "medium" if slag_val > 15 else "low",
                        "detail": f"渣损失率{slag_val}%，高于10%警戒线"})
        except Exception as e:
            logger.warning(f"FA 异常检测失败: {e}")

        overall_sev = "high" if any(a["severity"]=="high" for a in anomalies) else \
                      "medium" if any(a["severity"]=="medium" for a in anomalies) else "low"
        yield evt("step", {"step": "anomaly", "done": True,
        ...
        try:
            history_rows = (await session.execute(text(
                """SELECT id, batch_no, stage, summary, severity FROM production.ai_analysis
                   WHERE stage = :st AND message_seq = 0 AND is_deleted = false
                   ORDER BY created_at DESC LIMIT 3"""
            ), {"st": stage})).fetchall()
            ref_cases = [{"id": str(r.id), "batch_no": r.batch_no,
                "summary": r.summary, "severity": r.severity} for r in history_rows]
        except Exception:
            pass
        yield evt("step", {"step": "history", "done": True,
            "msg": f"找到 {len(ref_cases)} 个历史参考案例"})

        # Step 5: 构建 prompt + 调用 LLM
        prompt = FA_ANALYSIS_PROMPT.format(
            batch_no=batch_no, stage_label=stage_label,
            batch_data=batch_data, stats_data=stats_data or "暂无统计数据")
        ...
        try:
            ...
            async def _call_llm_stream(temp, extra=""):
                ...
                s = await client.chat.completions.create(
                    ...
                    temperature=temp, max_tokens=2000, timeout=120, stream=True,
                    extra_body={"thinking": {"type": "disabled"}})
            ...
            summary = parsed.get("summary", ""); causes = parsed.get("causes", [])
            suggestions = parsed.get("suggestions", [])
            ...
            if len(causes) < 2 or len(suggestions) < 2:
                ...
                if retry.get("summary"): summary = retry["summary"]
                if retry.get("causes"): causes = retry["causes"]
                if retry.get("suggestions"): suggestions = retry["suggestions"]
                if retry.get("severity"): llm_severity = retry["severity"]
        except Exception as e:
            ...
            if not suggestions:
                for a in anomalies[:5]:
                    suggestions.append(f"关注 {a['stage']} {a['batch_no']}，{a['detail']}")
                if not suggestions:
        ...
        try:
            analysis = AiAnalysis(
                batch_no=batch_no, stage=stage, include_siblings=False,
                trace_snapshot={"batch_data": batch_data},
                dist_snapshot={"stats": stats_data},
                anomalies=anomalies, llm_prompt=prompt, llm_response=llm_text,
                summary=summary, causes=causes, suggestions=suggestions,
                severity=llm_severity, model_used=cfg.model_name or "",
                reference_cases=[c["id"] for c in ref_cases],
                created_by="AI自动分析", session_id=sid,
                message_seq=0, role="system")
        ...
        try:
            yield evt("result", {"id": analysis_id,
                "session_id": sid, "summary": summary, "anomalies": anomalies,
                "causes": causes, "suggestions": suggestions, "severity": llm_severity,
                "analysis_text": llm_text, "reference_cases": ref_cases})
