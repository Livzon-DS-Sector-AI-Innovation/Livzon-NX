...
FIELD_MAPPING = {"批次":"batch_no","烘干干品总重量(kg)":"feed_weight","来料批号":"incoming_batch","来料效价(U/g)":"incoming_titer","来料水分(%)":"incoming_moisture","杂质检测报告":"impurity_report","分装规格(kg/桶)":"pack_spec","包装桶数量(桶)":"barrel_count","每桶实际装料重量(kg)":"per_barrel_weight","总包装成品净重(kg)":"total_net_weight","取样复检重量(kg)":"sample_weight","留样重量(kg)":"retain_weight","不合格品重量(kg)":"reject_weight","筛分粉尘损耗(kg)":"screen_loss","分装撒料损耗(kg)":"spill_loss","成品总收率(%)":"total_yield","包装日期":"pack_date","操作人员":"operator","外包装编号":"outer_pack_no","入库数量(kg)":"warehouse_qty","备注":"remarks"}
...
async def sync_pack(config: ProductionFeishuConfig, session: AsyncSession) -> dict:
