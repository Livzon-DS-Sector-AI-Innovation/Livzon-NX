...
NUMBER_FIELDS = {"seq_no", "broth_od", "titer_u_ml", "titer_mg_l", "broth_ph", "temperature",
                  "mycelium_concentration", "residual_sugar", "amino_nitrogen",
                  "tank_bottom_residue", "receive_loss"}
...
async def sync_broth_receive(config: ProductionFeishuConfig, session: AsyncSession) -> dict:
    ...
    sql = text(f"INSERT INTO production.broth_receives (id, {', '.join(all_cols)}) VALUES "
               + ", ".join(values_clauses)
