...
class Conc2(BaseModel):
    __tablename__ = "conc2"
    __table_args__ = (
        Index("ix_c2_batch", "batch_no"),
        Index("ix_c2_frid", "feishu_record_id", unique=True),
        {"schema": "production"},
    )
    ...
    feed_volume: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="二次板框母液合并总量(L)"
    )
    vacuum_degree: Mapped[float | None] = mapped_column(
        Float, nullable=True, comment="真空(MPa)"
    )
    ...
    endpoint_density: Mapped[float | None] = mapped_column(
        Float, nullable=True, comment="浓缩终点比重"
    )
    conc_volume: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="二次浓缩液体积(L)"
    )
    conc_titer: Mapped[float | None] = mapped_column(
        Float, nullable=True, comment="二次浓缩液效价(U/mL)"
    )
    conc_factor: Mapped[float | None] = mapped_column(
        Float, nullable=True, comment="浓缩倍数"
    )
    condensate_volume: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="冷凝水量(L)"
    )
    bottom_residue: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="釜底残液(L)"
    )
    evap_loss_rate: Mapped[float | None] = mapped_column(
        Float, nullable=True, comment="挥发损耗(%)"
    )
