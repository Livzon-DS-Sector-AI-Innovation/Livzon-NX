...
REFINEMENT_SHEET = "4UmRTh"  # 二次精制台账
SHEET_RANGE = "A4:AB5000"  # 28 列
...
COL = {
    ...
    "batch_yield": 10,
    "remarks": 11,
    "recovery_powder_weight_kg": 12,
    "recovery_powder_pure_kg": 13,
    "mother_liquor_unit": 14,
    "mother_liquor_volume": 15,
    "mother_liquor_product_kg": 16,
    "impurity_6": 17,
    "impurity_1": 18,
    "impurity_2": 19,
    "impurity_7": 20,
...
STRING_COLS: set[str] = {"feed_batch_no", "remarks"}

_token_cache: dict[str, str] = {}


async def _get_token(app_id: str, app_secret: str) -> str:
    k = f"dr_refine2:{app_id}"
...
async def sync_dr_second_refinement(
    ...
    for ri, row in enumerate(rows):
        ...
        try:
            ...
            logger.warning(f"[DR二次精制同步] 行 {ri + DATA_START_ROW}: {e}")
            stats["errors"] += 1

    # 全量重建：先清空旧数据，再按飞书原始行序逐行插入（不做去重，忠实保留每一行）
    if data_rows:
        await session.execute(text("DELETE FROM production.dr_second_refinement"))
        for data in data_rows:
            cols = ", ".join(data.keys())
            vals = ", ".join(f":{k}" for k in data)
            await session.execute(
                text(
                    f"INSERT INTO production.dr_second_refinement "
    ...
    logger.info("[DR二次精制同步] 完成: %s", stats)
    return stats
