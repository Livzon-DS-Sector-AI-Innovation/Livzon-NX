...
from app.modules.production.mc_yield_anomaly_detector import (
    _compute_stage_iqr, judge_anomaly_severity,
)
...
async def _get_node_batch_date(session: AsyncSession, stage: str, batch_no: str) -> date | None:
    ...
    if stage in ("sub_tank", "crude_product"):
        row = (await session.execute(text(
            ...
            "JOIN production.refining_batches rb ON rb.batch_no = st.parent_batch AND rb.is_deleted = false "
...
async def _detect_yield_anomalies(
    session: AsyncSession, stages: list[dict],
) -> list[dict]:
    ...
    for sg in stages:
        ...
        for n in sg.get("nodes", []):
            ...
            try:
                ...
                anomalies.append({
                    "stage": stage_name, "batch_no": bn,
                    "metric": "yield_rate", "value": round(yr, 1),
                    "benchmark": round(iqr["median"], 1), "severity": sev,
                    "detail": f"{direction}移动窗口中位数{diff:.1f}个百分点(IQR={iqr['iqr']})",
                })
            except Exception:
...
async def ai_analyze(
    ...
    # 1. trace
    trace_resp = await lineage_trace(batch_no=batch_no, stage=stage,
        include_siblings=include_siblings, session=session)
    ...
    dist_map = {d.get("stage"): d for d in dist_data}
    ...
    if real_stage in ("blending", "qc", "single_batch_blend", "single_batch_qc"):
        for sg in stages:
            if sg.get("stage") != "blending": continue
            for n in sg.get("nodes", []):
                bn = n.get("batch_no", "")
                if not bn: continue
                row = (await session.execute(text("""
                ...
                if imp.get("total_impurity") and imp["total_impurity"] > 1.5:
                    anomalies.append({"stage": "blending", "batch_no": bn,
                        "metric": "total_impurity", "value": imp["total_impurity"],
                        "benchmark": 1.5, "severity": "high",
                        "detail": f"总杂{imp['total_impurity']}%, 超出1.5%"})
                for rrt_key in ("rrt_053", "rrt_0755", "rrt_094_096", "rrt_103_106", "rrt_201"):
                    val = imp.get(rrt_key)
                    if val and val > 0.5:
                        sev = "high" if val > 1.0 else "medium"
                        anomalies.append({"stage": "blending", "batch_no": bn,
                            "metric": rrt_key, "value": val, "benchmark": 0.5,
                            "severity": sev, "detail": f"{rrt_key}={val}%, 超出0.5%"})

    overall_sev = "high" if cumulative_yield < 70 else "medium" if cumulative_yield < 85 else "low"

    # 5. history
    ref_cases = []
    if anomalies:
        history = (await session.execute(text("""
            SELECT id, batch_no, stage, summary, causes, suggestions, severity, created_at
            FROM production.ai_analysis WHERE stage = :st AND is_deleted = false
            ORDER BY created_at DESC LIMIT 3
        """), {"st": real_stage})).fetchall()
        ref_cases = [{"id": str(r.id), "batch_no": r.batch_no, "summary": r.summary,
            "causes": r.causes, "suggestions": r.suggestions, "severity": r.severity} for r in history]

    # 6. prompt
    prompt = _build_prompt(batch_no, real_stage, stages, cumulative_yield,
                           max_loss_stage, anomalies, blend_impurities, ref_cases)

    # 7. LLM
    llm_text = ""; summary = ""; causes = []; suggestions = []; llm_severity = overall_sev
    try:
        ...
        async def _call_llm(temperature: float, extra_hint: str = "") -> str:
            ...
            resp = await client.chat.completions.create(
                ...
                messages=msgs, temperature=temperature, max_tokens=3000, timeout=120,
        ...
        if len(causes) < 3 or len(suggestions) < 3:
            logger.warning(f"AI 分析过于简短（causes={len(causes)}, suggestions={len(suggestions)}），重试中...")
            llm_text = await _call_llm(0.7, "\n\n【重要提醒】上一轮分析太简略。请逐工段详细评价：收率是否合理、操作控制情况、与历史均值对比。causes和suggestions各至少4条。")
            parsed = _parse_json(llm_text)
        ...
        if not causes:
            for a in (anomalies or []):
                causes.append(f"{a['stage']} {a['batch_no']} {a['metric']}={a['value']}，{a['detail']}")
            if not causes:
                causes.append("各工段收率均在正常范围内，无异常标记")
        if not suggestions:
            for a in (anomalies or []):
                suggestions.append(f"关注 {a['stage']} 工段 {a['batch_no']}，{a['detail']}，建议排查工艺参数")
            if not suggestions:
    ...
    sid = str(_uuid.uuid4())
    analysis = AiAnalysis(
        batch_no=batch_no, stage=real_stage, include_siblings=include_siblings,
        trace_snapshot=trace_data, dist_snapshot=dist_data, anomalies=anomalies,
        llm_prompt=prompt, llm_response=llm_text, summary=summary,
        causes=causes, suggestions=suggestions, severity=llm_severity,
        model_used=cfg.model_name or "", reference_cases=[c["id"] for c in ref_cases],
        created_by="AI自动分析",
        session_id=sid, message_seq=0, role="system")
    session.add(analysis); await session.commit()

    return success_response(data={"id": str(analysis.id), "session_id": sid, "summary": summary,
        "anomalies": anomalies, "causes": causes, "suggestions": suggestions,
        "severity": llm_severity, "analysis_text": llm_text, "reference_cases": ref_cases})


@router.get("/mc/lineage/ai-analysis-stream", summary="AI 分析（SSE 流式，展示思考过程）")
async def ai_analyze_stream(
    ...
    async def generate():
        ...
        sid = str(_uuid.uuid4())
        cfg = await get_config("text")
        evt = lambda t, d: f"data: {json.dumps({'type': t, **d}, ensure_ascii=False)}\n\n"

        yield evt("step", {"step": "trace", "msg": "正在查询批次追溯链路..."})

        # 1. trace
        trace_resp = await lineage_trace(batch_no=batch_no, stage=stage, include_siblings=include_siblings, session=session)
        trace_data = json.loads(trace_resp.body).get("data", {})
        stages = trace_data.get("stages", [])
        total_nodes = sum(len(sg.get("nodes", [])) for sg in stages)
        yield evt("step", {"step": "trace", "done": True, "msg": f"已获取 {len(stages)} 个工段、{total_nodes} 个节点"})
        ...
        dist_data = json.loads(dist_resp.body).get("data", [])
        dist_map = {d.get("stage"): d for d in dist_data}
        yield evt("step", {"step": "yield", "done": True, "msg": f"已获取 {len(dist_data)} 个工段的收率统计"})

        # 3. anomalies（3月移动窗口 IQR）
        yield evt("step", {"step": "anomaly", "msg": "正在用移动窗口IQR检测收率异常..."})
        anomalies = await _detect_yield_anomalies(session, stages)
        yield evt("step", {"step": "anomaly", "done": True, "msg": f"检测到 {len(anomalies)} 个收率异常"})
        ...
        if real_stage in ("blending", "qc", "single_batch_blend", "single_batch_qc"):
            for sg in stages:
                ...
                for n in sg.get("nodes", []):
                    ...
                    if not bn: continue
                    row = (await session.execute(text(
                        "SELECT rrt_053, rrt_0755, rrt_094_096, rrt_103_106, rrt_201, total_impurity FROM production.blending_records WHERE batch_no = :bn AND is_deleted = false"
                    ), {"bn": bn})).fetchone()
                    if not row: continue
                    imp = {"batch_no": bn}
                    for key in ("rrt_053", "rrt_0755", "rrt_094_096", "rrt_103_106", "rrt_201"):
                    ...
                    blend_impurities.append(imp)
                    if imp.get("total_impurity") and imp["total_impurity"] > 1.5:
                        anomalies.append({"stage": "blending", "batch_no": bn, "metric": "total_impurity", "value": imp["total_impurity"], "benchmark": 1.5, "severity": "high", "detail": f"总杂{imp['total_impurity']}%, 超出1.5%"})
                    for rrt_key in ("rrt_053", "rrt_0755", "rrt_094_096", "rrt_103_106", "rrt_201"):
                        val = imp.get(rrt_key)
                        if val and val > 0.5:
                            sev = "high" if val > 1.0 else "medium"
                            anomalies.append({"stage": "blending", "batch_no": bn, "metric": rrt_key, "value": val, "benchmark": 0.5, "severity": sev, "detail": f"{rrt_key}={val}%, 超出0.5%"})

        overall_sev = "high" if cumulative_yield < 70 else "medium" if cumulative_yield < 85 else "low"
        yield evt("step", {"step": "anomaly", "done": True, "msg": f"检测到 {len(anomalies)} 个异常点，严重程度: {overall_sev}"})

        # 4. ref cases
        yield evt("step", {"step": "history", "msg": "正在检索历史分析案例..."})
        ref_cases = []
        history_rows = (await session.execute(text(
            "SELECT id, batch_no, stage, summary, causes, suggestions, severity FROM production.ai_analysis WHERE stage = :st AND is_deleted = false ORDER BY created_at DESC LIMIT 3"
        ), {"st": real_stage})).fetchall()
        ref_cases = [{"id": str(r.id), "batch_no": r.batch_no, "summary": r.summary, "severity": r.severity} for r in history_rows]
        yield evt("step", {"step": "history", "done": True, "msg": f"找到 {len(ref_cases)} 个历史参考案例"})

        # 5. prompt
        prompt = _build_prompt(batch_no, real_stage, stages, cumulative_yield, max_loss_stage, anomalies, blend_impurities, ref_cases)

        # 6. LLM
        yield evt("step", {"step": "llm", "msg": "正在调用 AI 模型分析，请稍候..."})
        llm_text = ""; summary = ""; causes = []; suggestions = []; llm_severity = overall_sev
        try:
            client = AsyncOpenAI(base_url=cfg.api_base_url or "https://newapi.livzon.cn/v1", api_key=cfg.api_key or "")

            async def _call_llm_stream(temp, extra=""):
                nonlocal llm_text
                msgs = [{"role": "user", "content": prompt + extra}]
                s = await client.chat.completions.create(
                    model=cfg.model_name or "deepseek-v4-pro", messages=msgs,
                    temperature=temp, max_tokens=3000, timeout=120, stream=True,
                    extra_body={"thinking": {"type": "disabled"}})
            ...
            parsed = _parse_json(llm_text)
            summary = parsed.get("summary", ""); causes = parsed.get("causes", []); suggestions = parsed.get("suggestions", [])
            llm_severity = parsed.get("severity", overall_sev)

            if len(causes) < 3 or len(suggestions) < 3:
                yield evt("step", {"step": "llm_retry", "msg": "分析过短，重新生成..."})
                prev_text = llm_text
                async for e in _call_llm_stream(0.7, "\n\n【重要提醒】上一轮分析太简略。请逐工段详细评价。causes和suggestions各至少4条。"):
                    yield e
                parsed_retry = _parse_json(llm_text)
                if not parsed_retry.get("summary") and prev_text:
                    parsed_retry = _parse_json(prev_text)
                if parsed_retry.get("summary"): summary = parsed_retry["summary"]
                if parsed_retry.get("causes"): causes = parsed_retry["causes"]
                if parsed_retry.get("suggestions"): suggestions = parsed_retry["suggestions"]
                if parsed_retry.get("severity"): llm_severity = parsed_retry["severity"]
        except Exception as e:
            logger.error(f"LLM stream failed: {e}")
            llm_text = f"分析失败: {e}"
            if not causes:
                for a in anomalies: causes.append(f"{a['stage']} {a['batch_no']} {a['metric']}={a['value']}，{a['detail']}")
                if not causes: causes.append("各工段收率正常")
            if not suggestions:
                for a in anomalies: suggestions.append(f"关注 {a['stage']} {a['batch_no']}，{a['detail']}")
                if not suggestions: suggestions.append("持续监控各工段关键参数")
        ...
        analysis = AiAnalysis(
            ...
            causes=causes, suggestions=suggestions, severity=llm_severity,
            model_used=cfg.model_name or "", reference_cases=[c["id"] for c in ref_cases],
            created_by="AI自动分析", session_id=sid, message_seq=0, role="system")
        session.add(analysis); await session.commit()

        yield evt("result", {"id": str(analysis.id), "session_id": sid, "summary": summary,
            "anomalies": anomalies, "causes": causes, "suggestions": suggestions,
            "severity": llm_severity, "analysis_text": llm_text, "reference_cases": ref_cases,
            "trace_nodes": total_nodes, "trace_stages": len(stages)})
        yield evt("done", {})
...
async def ai_history(
    ...
    _STAGE_ALIASES = {"na_batch": "sub_tank", "crude_product": "sub_tank",
    ...
    """), {"bn1": clean_bn, "bn2": f"MC-{clean_bn}",
           "st1": stage, "st2": resolved_stage,
           "lim": limit})).fetchall()
    ...
    return success_response(data={"batch_no": batch_no, "stage": stage, "records": records, "total": len(records)})


def _build_prompt(batch_no, stage, stages, cumulative_yield, max_loss_stage, anomalies, impurities, ref_cases):
    label = STAGE_LABELS.get(stage, stage)

    # 找本批提炼批号（主线子罐的父批）
    target_refining = ""
    for sg in stages:
        if sg.get("stage") != "sub_tank": continue
        for n in sg.get("nodes", []):
            if n.get("is_sibling"): continue
            bn = n.get("batch_no", "")
    ...
    for sg in stages:
        for n in sg.get("nodes", []):
            if not n.get("is_sibling"): continue
            bn = n.get("batch_no", "")
    ...
    for sg in stages:
        for n in sg.get("nodes", []):
            ...
            if ext_batches and (n.get("batch_no") in ext_batches or
               (ext_active and n.get("is_sibling") and n.get("batch_no", "").startswith("MC-"))):
                if n.get("batch_no") in ext_batches: ext_active = True
    ...
    if not ext_lines:
        for sg in stages:
            for n in sg.get("nodes", []):
                if n.get("is_sibling"):
                    sib = " [同级]" if n.get("is_sibling") else ""
                    line = f"  {sg['label']} {n['batch_no']}({n.get('detail','')}){sib}"
                    main_lines.remove(line) if line in main_lines else None
                    ext_lines.append(line)

    lines = [f"你是制药工艺分析师，拥有霉酚酸(MC)生产线深度知识。分析中必须使用中文工段名称（如粗提分罐、萃取），禁止出现sub_tank/extraction/refinement等英文工段代码。\n\n=== 批次信息 ===\n目标: {batch_no}（{label}）\n累计收率: {cumulative_yield}%\n最大损失工段: {max_loss_stage or '无'}\n\n=== 本批全链路 ==="]
    lines.extend(main_lines)

    if ext_lines and ext_refining:
        lines.append(f"\n=== 外批复联（来自提炼批 {ext_refining}，非本批主线，因汇入同一混粉节点产生关联） ===")
    ...
    if impurities:
        ...
        for imp in impurities:
            ...
            for k in ("rrt_053", "rrt_0755", "rrt_094_096", "rrt_103_106", "rrt_201"):
                if imp.get(k) is not None: parts.append(f"{k}={imp[k]}%")
            if imp.get("total_impurity") is not None: parts.append(f"总杂={imp['total_impurity']}%")
    ...
    if anomalies:
        ...
        for a in anomalies:
            ...
            lines.append(f"⚠ {stage_cn} {a['batch_no']} {a['metric']}={a['value']} — {a['detail']}")
...
def _parse_json(text):
    ...
    if m: result["severity"] = m.group(1)
    # causes/suggestions 用正则提取已完成的条目
    for key in ("causes", "suggestions"):
        items = re.findall(r'"([^"]*)"', t[t.find(f'"{key}"'):t.find(']', t.find(f'"{key}"'))] if f'"{key}"' in t else "")
