...
from app.modules.production.fa_models import FaFermentationBatch, FaFermentationSubBatch, FaAcidificationRecord, FaDecolor1Record
...
async def list_batches(
    ...
    # 查询条件
    q = select(FaFermentationBatch).where(
        FaFermentationBatch.is_deleted == False
    ).order_by(FaFermentationBatch.放罐日期.desc().nulls_last())
    count_q = select(func.count()).select_from(FaFermentationBatch).where(
        FaFermentationBatch.is_deleted == False
    )
    ...
    if tank_nos:
        sub_q = select(FaFermentationSubBatch).where(
            FaFermentationSubBatch.父发酵罐号.in_(tank_nos),
            FaFermentationSubBatch.is_deleted == False,
...
@router.get("/fa/fermentation/flat-list", summary="发酵液放罐 — 平铺列表（飞书原表格式）")
async def list_flat(
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=200),
    tank_no: str | None = Query(None, description="罐号筛选"),
    ...
    # 按主批分页
    b_q = select(FaFermentationBatch).where(
        FaFermentationBatch.is_deleted == False
    )
    count_q = select(func.count()).select_from(FaFermentationBatch).where(
        FaFermentationBatch.is_deleted == False
    ...
    if month is not None:
        b_q = b_q.where(text("CASE WHEN EXTRACT(DAY FROM \"放罐日期\") >= 27 THEN CASE WHEN EXTRACT(MONTH FROM \"放罐日期\") = 12 THEN 1 ELSE EXTRACT(MONTH FROM \"放罐日期\") + 1 END ELSE EXTRACT(MONTH FROM \"放罐日期\") END = %s" % month))
        count_q = count_q.where(text("CASE WHEN EXTRACT(DAY FROM \"放罐日期\") >= 27 THEN CASE WHEN EXTRACT(MONTH FROM \"放罐日期\") = 12 THEN 1 ELSE EXTRACT(MONTH FROM \"放罐日期\") + 1 END ELSE EXTRACT(MONTH FROM \"放罐日期\") END = %s" % month))
...
async def get_batch(
    tank_no: str, session: AsyncSession = Depends(get_db),
):
...
async def update_sub_batch(
    sub_id: str, data: dict, session: AsyncSession = Depends(get_db),
):
...
async def list_acidification(
    ...
    if month is not None:
        q = q.where(text("CASE WHEN EXTRACT(DAY FROM \"日期\") >= 27 THEN CASE WHEN EXTRACT(MONTH FROM \"日期\") = 12 THEN 1 ELSE EXTRACT(MONTH FROM \"日期\") + 1 END ELSE EXTRACT(MONTH FROM \"日期\") END = %s" % month))
        count_q = count_q.where(text("CASE WHEN EXTRACT(DAY FROM \"日期\") >= 27 THEN CASE WHEN EXTRACT(MONTH FROM \"日期\") = 12 THEN 1 ELSE EXTRACT(MONTH FROM \"日期\") + 1 END ELSE EXTRACT(MONTH FROM \"日期\") END = %s" % month))
...
async def list_decolor1(
    ...
    for r in rows:
        ...
        items.append({
            "id": r.id, "日期": date_str, "批号": r.批号,
            "体积(kl)": r.体积_kl, "含量(g/L)": r.含量_gL, "电导(us/cm)": r.电导_uscm,
            "调前电导碳柱(us/cm)": r.调前电导碳柱, "混合含量(g/L)": r.混合含量_gL,
            "母液体积(kl)": r.母液体积_kl, "母液含量(g/L)": r.母液含量_gL,
            "电导(us/cm)2": r.电导2, "活性炭添加量(kg)": r.活性炭添加量_kg,
            "碳后含量(g/L)": r.碳后含量_gL, "湿重(kg）": r.湿碳_kg,
            "收率": r.收率, "产品量(kg)": r.产品量_kg, "滤损失率": r.滤损失率, "备注": r.备注,
        })
...
async def list_mvr(
    ...
    for r in rows:
        date_str = f"{r.日期.month}月{r.日期.day}日" if r.日期 else None
        items.append({"id":r.id,"日期":date_str,"白班进料/m3":r.白班进料,"白班出料/m3":r.白班出料,
            "白班进料合计/m3":r.白班进料合计,"白班进料累计合计/m3":r.白班进料累计合计,
            "夜班进料/m3":r.夜班进料,"夜班出料/m3":r.夜班出料,"夜班进料合计/m3":r.夜班进料合计,
            "夜班进料累计合计/m3":r.夜班进料累计合计,"备注":r.备注})
    return success_response({"items":items,"total":total,"page":page,"page_size":page_size})
...
async def list_mother_liquor(
    ...
    for r in rows:
        date_str = f"{r.日期.month}月{r.日期.day}日" if r.日期 else None
        items.append({"id":r.id,"日期":date_str,"批号":r.批号,"母液打料量(m3)":r.母液打料量,
            "溶解体积(m3)":r.溶解体积,"溶解含量(g/L)":r.溶解含量,"电导(ms/cm)":r.电导,"ph":r.ph,"备注":r.备注})
    return success_response({"items":items,"total":total,"page":page,"page_size":page_size})
...
async def list_plate_recovery(
    ...
    for r in rows:
        ...
        items.append({"id":r.id,"日期":ds,"白班板框进料量/方":r.白班板框进料量,"白班板框拆卸回收粉包数":r.白班板框拆卸回收粉包数,
            "白班分液罐投回收粉包数/包":r.白班分液罐投回收粉包数,"白班分液罐体积/方":r.白班分液罐体积,
            "复滤粉拆包数":r.复滤粉拆包数,"夜班板框进料量/方":r.夜班板框进料量,"夜班板框拆卸回收粉包数":r.夜班板框拆卸回收粉包数,
            "夜班分液罐投回收粉包数/包":r.夜班分液罐投回收粉包数,"夜班分液罐体积/方":r.夜班分液罐体积,
            "复滤粉拆包数(夜)":r.复滤粉拆包数夜,"白班装车体积":r.白班装车体积,"废液槽接收体积":r.废液槽接收体积,
            "总进料体积（m3/天）":r.总进料体积,"累计进料体积m3":r.累计进料体积})
    return success_response({"items":items,"total":total,"page":page,"page_size":page_size})
...
async def list_decolor_centrifuge(
    ...
    where = "" if month is None else " WHERE CASE WHEN EXTRACT(DAY FROM \"日期\") >= 27 THEN CASE WHEN EXTRACT(MONTH FROM \"日期\") = 12 THEN 1 ELSE EXTRACT(MONTH FROM \"日期\") + 1 END ELSE EXTRACT(MONTH FROM \"日期\") END = %s" % month
    params = {"limit": page_size, "offset": (page-1)*page_size}
    q = text(f"SELECT * FROM production.fa_decolor_centrifuge_records{where} ORDER BY id LIMIT :limit OFFSET :offset")
    total = (await session.execute(text(f"SELECT count(*) FROM production.fa_decolor_centrifuge_records{where}"), params)).scalar() or 0
    ...
    for r in rows:
        ...
        items.append({
            "id":r["id"],"日期":ds,"批号":r.get("批号"),
            "进料体积（kl）":r.get("进料体积（kl）"),"出料体积（kl）":r.get("出料体积（kl）"),
            "顶洗时长（min）":r.get("顶洗时长（min）"),"甩料车数":r.get("甩料车数"),"水分（%）":r.get("水分（%）"),
            "体积（kl）":r.get("体积（kl）"),"炭脱PH":r.get("炭脱PH"),"炭前真实含量（g/L）":r.get("炭前真实含量（g/L）"),"炭前总量":r.get("炭前总量"),
            "活性炭用量（kg)":r.get("活性炭用量（kg)"),"活性炭品牌":r.get("活性炭品牌"),"炭后真实含量(g/L）":r.get("炭后真实含量(g/L）"),
            "透光（%）":r.get("透光（%）"),"亚硫酸氢钠（kg）":r.get("亚硫酸氢钠（kg）"),
            "顶洗时长（min)2":r.get("顶洗时长（min)2"),"收率":r.get("收率"),
            "二次离心_批号":r.get("二次离心_批号"),"二次离心_甩料车数":r.get("二次离心_甩料车数"),"二次离心_顶洗次数":r.get("二次离心_顶洗次数"),
        })
    return success_response({"items":items,"total":total,"page":page,"page_size":page_size})
...
async def list_intermediate(
    ...
    where = "" if month is None else " WHERE CASE WHEN EXTRACT(DAY FROM \"日期\") >= 27 THEN CASE WHEN EXTRACT(MONTH FROM \"日期\") = 12 THEN 1 ELSE EXTRACT(MONTH FROM \"日期\") + 1 END ELSE EXTRACT(MONTH FROM \"日期\") END = %s" % month
    params = {"limit": page_size, "offset": (page-1)*page_size}
    q = stext(f"SELECT * FROM production.fa_intermediate_records{where} ORDER BY id LIMIT :limit OFFSET :offset")
    total = (await session.execute(stext(f"SELECT count(*) FROM production.fa_intermediate_records{where}"), params)).scalar() or 0
    rows = (await session.execute(q, params)).mappings().all()
    items = []
    for r in rows:
        d = r.get("日期")
        ds = f"{d.month}月{d.day}日" if hasattr(d, 'month') and d else None
        items.append({
            "id":r["id"],"日期":ds,"当日母液总体积/方":r.get("当日母液总体积/方"),"顶水回流/方6#板框":r.get("顶水回流/方6#板框"),
            "当日结晶液产母液量（方）":r.get("当日结晶液产母液量（方）"),"一次离心日用水量（方）":r.get("一次离心日用水量（方）"),
            "一次甩料车数":r.get("一次甩料车数"),"离心每车平均用水量（L)160":r.get("离心每车平均用水量（L)160"),
            "三效产生一次母液量（方）":r.get("三效产生一次母液量（方）"),"三效单车产母液量(L)410":r.get("三效单车产母液量(L)410"),
            "合计570":r.get("合计570"),"二次母液总量":r.get("二次母液总量"),"二次离心日用水量（方）":r.get("二次离心日用水量（方）"),
            "二次甩料车数":r.get("二次甩料车数"),"离心每车平均用水量(L)170左右":r.get("离心每车平均用水量(L)170左右"),
            "合计750":r.get("合计750")})
    return success_response({"items":items,"total":total,"page":page,"page_size":page_size})
...
AVG_TABLES = {
    "fermentation_batches": ("fa_fermentation_monthly", "放罐日期"),  # 视图：JOIN 子批表取体积/含量
...
async def monthly_averages(
    ...
    num_cols = [r[0] for r in col_rows if r[0] not in ("created_at", "updated_at", "is_deleted")]
    if not num_cols: return success_response({"data": [], "columns": []})
...
async def trigger_fa_sync(
    ...
    modules = data.get("modules", [
        "fermentation", "acidification", "decolor1", "mvr",
        "mother_liquor", "plate_recovery", "decolor_centrifuge", "intermediate"
    ])
    ...
    return success_response({"results": results, "errors": len(errors)},
