...
class CD(BaseModel):
    ...
    sep_duration: float | None = None
    feed_flow: float | None = None
    crystal_wet_weight: str | None = None
    waste_liquor_volume: str | None = None
    mother_liquor_titer: float | None = None
    crystal_moisture: float | None = None
    liquor_recovery: str | None = None
    crystal_yield: float | None = None
...
async def _list(
    ...
    rows = await session.execute(
        q.order_by(Centrifuge2.seq_no.asc()).offset((page - 1) * ps).limit(ps)
    )
