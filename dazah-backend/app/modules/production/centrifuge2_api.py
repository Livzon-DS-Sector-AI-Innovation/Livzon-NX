...
from app.modules.production.centrifuge2_models import Centrifuge2
from app.shared.module_api import create_module_router
from app.shared.module_registry import MODULES_BY_CODE
router = create_module_router(MODULES_BY_CODE["production"])

class CD(BM): seq_no:O[int]=None; batch_no:str; feed_volume:O[str]=None; rotation_speed:O[float]=None; sep_duration:O[float]=None; feed_flow:O[float]=None; crystal_wet_weight:O[str]=None; waste_liquor_volume:O[str]=None; mother_liquor_titer:O[float]=None; crystal_moisture:O[float]=None; liquor_recovery:O[str]=None; crystal_yield:O[float]=None; remarks:O[str]=None
class RD(BM): id:UUID; seq_no:O[int]=None; batch_no:str; feed_volume:O[str]=None; rotation_speed:O[float]=None; sep_duration:O[float]=None; feed_flow:O[float]=None; crystal_wet_weight:O[str]=None; waste_liquor_volume:O[str]=None; mother_liquor_titer:O[float]=None; crystal_moisture:O[float]=None; liquor_recovery:O[str]=None; crystal_yield:O[float]=None; remarks:O[str]=None; created_at:dt; updated_at:dt; model_config={"from_attributes":True}

@router.get("/centrifuge2", summary="二次离心列表")
async def _list(page:int=Query(1,ge=1),ps:int=Query(20,ge=1,le=200),s:str|None=Query(None,alias="batch_no"),workshop:str=Query('203'),session:AsyncSession=Depends(get_db)):
    q=select(Centrifuge2).where(Centrifuge2.is_deleted==False, Centrifuge2.workshop==workshop)
    if s: q=q.where(Centrifuge2.batch_no.ilike(f"%{s}%"))
    total=(await session.execute(select(func.count()).select_from(q.subquery()))).scalar_one()
    rows=await session.execute(q.order_by(Centrifuge2.seq_no.asc()).offset((page-1)*ps).limit(ps))
...
async def _create(d:CD,session:AsyncSession=Depends(get_db)):
    r=Centrifuge2(**d.model_dump());session.add(r);await session.flush();await session.commit();await session.refresh(r)
