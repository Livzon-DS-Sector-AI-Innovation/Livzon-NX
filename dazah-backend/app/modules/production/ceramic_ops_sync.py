"""飞书 → ceramic_membrane_ops 全量同步（INSERT/UPDATE/DELETE）"""
import logging
...
FIELD_MAPPING = {"日期":"run_date","批次号":"batch_no","膜组件编号":"membrane_no","运行压力(MPa)":"run_pressure","膜面流速(m/s)":"membrane_velocity","跨膜压差(MPa)":"tmp","运行温度(°C)":"run_temp","渗透通量(L/m²·h)":"permeate_flux","操作人":"operator","备注":"remarks"}
NUMBER_FIELDS = {"run_pressure","membrane_velocity","tmp","run_temp","permeate_flux"}
DATE_FIELDS = {"run_date"}
TABLE = "ceramic_membrane_ops"
...
async def sync_ceramic_ops(config: ProductionFeishuConfig, session: AsyncSession) -> dict:
