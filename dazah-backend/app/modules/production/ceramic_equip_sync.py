...
FIELD_MAPPING = {"日期":"record_date","设备编号":"equipment_no","运行状态":"run_status","异常类型":"abnormal_type","异常描述":"abnormal_desc","处理措施":"action_taken","处理结果":"action_result","处理人":"handler","恢复时间":"restore_time","备注":"remarks"}
NUMBER_FIELDS = {}
DATE_FIELDS = {"record_date","restore_time"}
TABLE = "ceramic_equipment_logs"
...
async def sync_ceramic_equip(config: ProductionFeishuConfig, session: AsyncSession) -> dict:
    ...
    for item in items:
        ...
        if not mapped.get("equipment_no"): continue
