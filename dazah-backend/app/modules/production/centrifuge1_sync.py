...
FIELD_MAPPING = {
    "批次": "batch_no",
    "浓缩液进料总量(L)": "feed_volume",
    "固含量(%)": "solid_content",
    "进料温度(℃)": "feed_temp",
    "转速(rpm)": "rotation_speed",
    "离心时长(min)": "centrifuge_duration",
    "进料流量(L/h)": "feed_flow",
    "分离温度(℃)": "sep_temp",
    "上清液总量(L)": "supernatant_volume",
    "上清液效价(U/mL)": "supernatant_titer",
    "固体渣重量(kg)": "solid_waste_weight",
    "渣效价(U/g)": "waste_titer",
    "渣含水率(%)": "waste_moisture",
    "离心上清收率(%)": "centrifuge_yield",
    "固体废渣产出量(kg)": "solid_waste_output",
    "备注": "remarks",
}
NUMBER_FIELDS = {
    "solid_content",
...
async def sync_centrifuge1(
    ...
    if unmatched:
        logger.warning(
            "centrifuge1 同步警告：以下 %d 个映射字段在飞书中未找到数据：%s",
            len(unmatched),
            unmatched,
        )
