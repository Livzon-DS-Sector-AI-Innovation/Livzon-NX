...
def get_seed_culture_service(session: AsyncSession = Depends(get_db)) -> SeedCultureService:
...
async def list_seed_cultures(
    ...
    items, total = await svc.list_records(page=page, page_size=page_size, batch_no=batch_no, product_name=product_name)
    return paginated_response([SeedCultureResponse.model_validate(i) for i in items], page, page_size, total)


@router.get("/seed-cultures/{record_id}", summary="种子培养记录详情")
async def get_seed_culture(record_id: UUID, svc: SeedCultureService = Depends(get_seed_culture_service)):
...
async def create_seed_culture(data: SeedCultureCreate, svc: SeedCultureService = Depends(get_seed_culture_service)):
    record = await svc.create_record(data.model_dump())
    return success_response(SeedCultureResponse.model_validate(record), message="创建成功")


@router.put("/seed-cultures/{record_id}", summary="更新种子培养记录")
async def update_seed_culture(record_id: UUID, data: SeedCultureUpdate, svc: SeedCultureService = Depends(get_seed_culture_service)):
    record = await svc.update_record(record_id, data.model_dump(exclude_unset=True))
    return success_response(SeedCultureResponse.model_validate(record), message="更新成功")


@router.delete("/seed-cultures/{record_id}", summary="删除种子培养记录")
async def delete_seed_culture(record_id: UUID, svc: SeedCultureService = Depends(get_seed_culture_service)):
