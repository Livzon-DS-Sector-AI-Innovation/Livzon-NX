...
from app.modules.production.mc_lineage_api import STAGE_LABELS
...
async def scan_new_batches(session: AsyncSession) -> list[dict]:
    ...
    rows_raw = (await session.execute(text("""
        ...
        WHERE single_step_yield IS NOT NULL AND single_step_yield > 0 AND is_deleted = false
        AND NOT EXISTS (
            SELECT 1 FROM production.ai_analysis aa
            WHERE aa.batch_no = mc_refinement_records.batch_no AND aa.stage = 'refinement'
            AND aa.created_by = 'auto' AND aa.is_deleted = false
        )
        ORDER BY stage, batch_no
    """))).fetchall()
    return [{
        "stage": r.stage, "batch_no": r.batch_no,
...
async def _compute_stage_iqr(
    session: AsyncSession, stage: str, near_date: date | None,
) -> dict:
    ...
    if stage == "sub_tank":
        subquery = f"""
            ...
            JOIN production.refining_batches rb ON rb.batch_no = st.parent_batch AND rb.is_deleted = false
            WHERE st.{yield_col} IS NOT NULL AND st.{yield_col} > 0 AND st.is_deleted = false
    ...
    rows = (await session.execute(text(f"""
        ...
        FROM ({subquery}) t
    """), {"ws": window_start, "nd": near_date})).fetchone()

    q1 = float(rows.q1 or 0)
    q3 = float(rows.q3 or 0)
    median = float(rows.median or 0)
    iqr = round(q3 - q1, 1)
    return {"n": rows.n or 0, "median": median, "q1": q1, "q3": q3, "iqr": iqr,
            "window_start": window_start.isoformat(), "window_end": near_date.isoformat()}


async def _compute_similar_range(session: AsyncSession, stage: str, yield_rate: float) -> dict:
    """同工段收率 ±2% 范围内的历史批次统计"""
    table, yield_col = STAGE_TABLE[stage]
    ...
    """), {"low": low, "high": high})).fetchone()
    return {"count": row.cnt or 0, "avg": float(row.avg_yr or 0),
            "min": float(row.min_yr or 0), "max": float(row.max_yr or 0)}


async def _compute_equipment_trend(session: AsyncSession, stage: str, batch_no: str) -> list[dict]:
    ...
    params = {}
    if stage == "refinement":
        tank_row = (await session.execute(text(
            "SELECT dissolution_tank, crystallization_tank FROM production.mc_refinement_records "
        ...
        if tank_row and (tank_row.dissolution_tank or tank_row.crystallization_tank):
            ...
            tank_filter = "AND (dissolution_tank = :tank OR crystallization_tank = :tank)"
            params["tank"] = tank
...
async def _compute_downstream_comparison(session: AsyncSession, stage: str, batch_no: str) -> list[dict]:
    ...
    for r in ds_rows:
        ...
        if ds_stage in STAGE_TABLE:
            ...
            mean_row = (await session.execute(text(f"""
                SELECT ROUND(AVG(y)::numeric, 1) AS mean_yr, COUNT(*) AS n
                FROM (
                    SELECT {ds_yield_col} AS y FROM production.{ds_table}
                    WHERE {ds_yield_col} IS NOT NULL AND {ds_yield_col} > 0 AND is_deleted = false
                ) t
            """))).fetchone()
...
async def _get_similar_cases(session: AsyncSession, stage: str, severity: str, limit: int = 3) -> list[dict]:
    rows = (await session.execute(text(
        "SELECT id, batch_no, stage, summary, severity FROM production.ai_analysis "
        "WHERE stage = :st AND severity = :sev AND message_seq = 0 AND is_deleted = false "
        "ORDER BY created_at DESC LIMIT :lim"
    ), {"st": stage, "sev": severity, "lim": limit})).fetchall()
    return [{"id": str(r.id), "batch_no": r.batch_no, "stage": r.stage,
             "summary": r.summary, "severity": r.severity} for r in rows]
...
def _build_auto_detect_prompt(
    batch_no: str, stage: str, yield_rate: float, severity: str,
    stage_iqr: dict, similar_range: dict, equipment_trend: list[dict],
    downstream_comparison: list[dict], ref_cases: list[dict],
) -> str:
    ...
    window_info = f"（移动窗口: {stage_iqr.get('window_start','?')} → {stage_iqr.get('window_end','?')}）"

    lines = [
        f"你是制药工艺分析师，拥有霉酚酸(MC)生产线深度知识。分析中必须使用中文工段名称（如粗提分罐、萃取、精制），禁止出现sub_tank/extraction/refinement等英文工段代码。请针对自动检测到的收率异常进行分析。",
        "",
        "=== 批次信息 ===",
        f"工段: {label}({stage})",
        f"批号: {batch_no}",
    ...
    if similar_range["count"] > 0:
        lines.extend([
            "",
            "=== 同收率范围(±2%)历史批次 ===",
            f"共 {similar_range['count']} 批，收率范围 [{similar_range['min']}%, {similar_range['max']}%]",
...
async def _call_llm_and_save(
    ...
    prompt = _build_auto_detect_prompt(
        batch_no, stage, yield_rate, severity,
        stage_iqr, similar_range, equipment_trend,
        downstream_comparison, ref_cases,
    )

    llm_text = ""; summary = ""; causes = []; suggestions = []
    try:
        client = AsyncOpenAI(
        ...
        resp = await client.chat.completions.create(
            ...
            messages=msgs, temperature=0.3, max_tokens=3000, timeout=120,
        ...
        if len(causes) < 3 or len(suggestions) < 3:
            ...
            resp2 = await client.chat.completions.create(
                model=cfg.model_name or "deepseek-v4-pro",
                messages=msgs2, temperature=0.7, max_tokens=3000, timeout=120,
                extra_body={"thinking": {"type": "disabled"}},
            )
            llm_text = resp2.choices[0].message.content or ""
            parsed2 = _parse_json(llm_text)
            if parsed2.get("summary"): summary = parsed2["summary"]
            if parsed2.get("causes"): causes = parsed2["causes"]
            if parsed2.get("suggestions"): suggestions = parsed2["suggestions"]
    except Exception as e:
        logger.error(f"[异常检测] LLM 调用失败 {batch_no}: {e}")
        summary = f"{label}{batch_no} 收率{yield_rate}%异常(LLM调用失败)"
        causes = [f"收率{yield_rate}%, 低于中位数{stage_iqr['median']}%", "LLM调用失败,无法自动分析"]
        suggestions = ["建议人工复核该批次收率数据", "检查飞书台账原始记录是否准确"]

    sid = str(_uuid.uuid4())
    label = STAGE_LABELS_ZH.get(stage, stage)
    anomalies = [{
        "stage": label, "batch_no": batch_no,
        "metric": "yield_rate", "value": round(yield_rate, 1),
        "benchmark": round(stage_iqr["median"], 1),
        "severity": severity,
        "detail": (f"收率{yield_rate}%{'低于' if yield_rate < stage_iqr['median'] else '高于'}"
                   f"移动窗口中位数{stage_iqr['median']}%, IQR={stage_iqr['iqr']}"),
    }]

    analysis = AiAnalysis(
        batch_no=batch_no, stage=stage,
        include_siblings=False,
        trace_snapshot=None, dist_snapshot=None,
        anomalies=anomalies,
        llm_prompt=prompt, llm_response=llm_text,
        summary=summary, causes=causes, suggestions=suggestions,
        severity=severity,
        model_used=cfg.model_name or "",
        reference_cases=[c["id"] for c in ref_cases],
        created_by="auto", session_id=sid,
...
async def run_anomaly_detection(session: AsyncSession) -> dict:
    ...
    if not new_batches:
        return {"scanned": 0, "detected": 0, "high": 0, "medium": 0,
                "skipped_normal": 0, "errors": 0, "details": []}
    ...
    detected = 0; high_count = 0; medium_count = 0
    skipped_normal = 0; errors = 0
    details = []

    for batch in new_batches:
        try:
            stage = batch["stage"]; yr = batch["yield_rate"]; bn = batch["batch_no"]
            batch_date = batch.get("batch_date")

            stage_stats = await _get_iqr(stage, batch_date)

            if stage_stats["n"] < MIN_SAMPLE_SIZE:
                logger.info(f"[异常检测] {stage} 窗口样本不足 ({stage_stats['n']}<{MIN_SAMPLE_SIZE}), 跳过 {bn}")
                skipped_normal += 1
                continue

            severity = judge_anomaly_severity(yr, stage_stats["median"], stage_stats["iqr"])
            if severity is None:
                skipped_normal += 1
                continue
            ...
            await _call_llm_and_save(
                session, bn, stage, yr, severity,
                stage_stats, similar_range, equipment_trend, downstream_comp, ref_cases,
            )
            details.append({"batch_no": bn, "stage": stage, "severity": severity, "yield_rate": yr})
        except Exception as e:
            logger.exception(f"[异常检测] 批次 {batch.get('batch_no','?')} 检测失败: {e}")
            errors += 1

    return {
        "scanned": len(new_batches), "detected": detected,
        "high": high_count, "medium": medium_count,
        "skipped_normal": skipped_normal, "errors": errors,
        "details": details,
    }
