...
FIELD_MAPPING = {"批次":"batch_no","板框滤清液进料体积(L)":"feed_volume","板框滤清液进料效价(U/mL)":"feed_titer","板框滤清液进料温度(℃)":"feed_temp","浓缩真空度(MPa)":"vacuum_degree","蒸发温度(℃)":"evap_temp","蒸汽压力(MPa)":"steam_pressure","浓缩时长(h)":"conc_duration","冷凝水产出量(L)":"condensate_volume","浓缩终点比重(g/cm³)":"endpoint_density","浓缩终点折光(%)":"endpoint_refraction","浓缩终点体积(L)":"endpoint_volume","浓缩后料液重量(kg)":"conc_weight","浓缩后料液体积(L)":"conc_volume","浓缩后效价(U/mL)":"conc_titer","浓缩倍数(倍)":"conc_factor","蒸发损耗量(L)":"evap_loss","釜壁粘壁残料量(L)":"wall_residue","浓缩收率(%)":"conc_yield","备注":"remarks"}
NUMBER_FIELDS = {"feed_titer","feed_temp","vacuum_degree","evap_temp","steam_pressure","conc_duration","endpoint_density","endpoint_refraction","conc_titer","conc_factor","conc_yield"}
DATE_FIELDS = {}
TABLE = "conc1"
...
async def sync_conc1(config: ProductionFeishuConfig, session: AsyncSession) -> dict:
    ...
    if unmatched:
        logger.warning("conc1 同步警告：以下 %d 个映射字段在飞书中未找到数据：%s", len(unmatched), unmatched)
