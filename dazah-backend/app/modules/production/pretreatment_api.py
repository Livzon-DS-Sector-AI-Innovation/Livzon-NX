...
from app.modules.production.pretreatment_schemas import (
    PretreatmentCreate,
    PretreatmentResponse,
    PretreatmentUpdate,
)
...
async def list_pretreatments(
    ...
    rows = await session.execute(
        query.order_by(Pretreatment.seq_no.asc())
        .offset((page - 1) * page_size)
        .limit(page_size)
    )
    return paginated_response(
        [PretreatmentResponse.model_validate(r) for r in rows.scalars().all()],
        page,
        page_size,
        total,
    )
...
async def update_pretreatment(
    ...
    for k, v in data.model_dump(exclude_unset=True).items():
        setattr(r, k, v)
    await session.flush()
    return success_response(PretreatmentResponse.model_validate(r), message="更新成功")
