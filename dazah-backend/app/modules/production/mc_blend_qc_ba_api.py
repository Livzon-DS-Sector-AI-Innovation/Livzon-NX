...
from app.modules.production.mc_qc_ba_models import (
    QcInspection, QcInspectionItem, QcInspectionInput,
    ButylAcetateRecord,
)
...
async def full_list_blending(
    ...
    month: int | None = Query(None, description="筛选月份 (1-12)，按批号 MC-YYMMxx 的 MM 位匹配"),
    ...
    main_q = select(BlendingRecord).where(*conditions).order_by(BlendingRecord.batch_no.asc())
    ...
    result = []
    if batch_nos:
        inputs_q = select(BlendingInput).where(
...
async def list_blending(
    ...
    query = select(BlendingRecord).where(BlendingRecord.is_deleted == False, BlendingRecord.workshop == workshop)
    if batch_no:
        query = query.where(BlendingRecord.batch_no.ilike(f"%{batch_no}%"))
    count_q = select(func.count()).select_from(query.subquery())
    total = (await session.execute(count_q)).scalar_one()
    rows = await session.execute(query.order_by(BlendingRecord.batch_no.asc()).offset((page - 1) * page_size).limit(page_size))
    return paginated_response([_clean_dict(r) for r in rows.scalars().all()], page, page_size, total)
...
async def update_blending(record_id: UUID, data: dict, session: AsyncSession = Depends(get_db)):
...
async def delete_blending(record_id: UUID, session: AsyncSession = Depends(get_db)):
    record = await session.get(BlendingRecord, record_id)
    if not record: return success_response(None, message="记录不存在", status_code=404)
...
async def list_blending_inputs(batch_no: str, session: AsyncSession = Depends(get_db)):
    query = select(BlendingInput).where(BlendingInput.is_deleted == False, BlendingInput.blend_batch == batch_no)
...
async def delete_blending_input(record_id: UUID, session: AsyncSession = Depends(get_db)):
...
async def calculate_blending_impurities(batch_no: str, session: AsyncSession = Depends(get_db)):
    """根据投入明细计算加权平均杂质（5个RRT点位 + 总杂 + 含量）"""
    inputs_result = await session.execute(
        select(BlendingInput).where(BlendingInput.is_deleted == False, BlendingInput.blend_batch == batch_no)
    )
    inputs = inputs_result.scalars().all()
    if not inputs:
        return success_response(None, message="没有投入明细", status_code=400)

    fields = ["rrt_053", "rrt_0755", "rrt_094_096", "rrt_103_106", "rrt_201", "total_impurity", "content"]
    result = {}
    total_weight = sum(inp.input_weight for inp in inputs)

    for field in fields:
        weighted_sum = sum((getattr(inp, field) or 0) * inp.input_weight for inp in inputs)
        result[field] = round(weighted_sum / total_weight, 4) if total_weight > 0 else 0

    # 更新主表计算结果
    main = await session.execute(
        select(BlendingRecord).where(BlendingRecord.batch_no == batch_no, BlendingRecord.is_deleted == False)
    )
    ...
    if main:
        ...
        if main.pack_spec:
            try:
                ...
                main.barrel_count = int(main.total_weight / spec_kg) + (1 if main.total_weight % spec_kg > 0 else 0) if main.total_weight else None
            except: pass
        main.status = 2  # 计算完成
        await session.commit()

    # 检查杂质超限警告
    limits = {"rrt_053": 0.05, "rrt_0755": 0.07, "rrt_094_096": 0.14, "rrt_103_106": 0.075, "rrt_201": 0.08, "total_impurity": 0.6}
    warnings = {k: round(v, 4) for k, v in result.items() if k in limits and v > limits[k]}
    if result.get("content") and result["content"] < 99:
        warnings["content"] = round(result["content"], 2)

    return success_response({"impurities": result, "total_weight": total_weight, "warnings": warnings})
...
async def full_list_qc(month: int | None = Query(None, ge=1, le=12), session: AsyncSession = Depends(get_db)):
    """返回QC检验+投入明细的嵌套结构"""
    ...
    result = []
    if batch_nos:
        inputs_q = select(QcInspectionInput).where(
...
async def list_qc_inputs(qc_batch: str, session: AsyncSession = Depends(get_db)):
    query = select(QcInspectionInput).where(QcInspectionInput.is_deleted == False, QcInspectionInput.qc_batch == qc_batch)
    rows = await session.execute(query)
    return success_response([{k: v for k, v in _clean_dict(r).items() if not k.startswith('_')} for r in rows.scalars().all()])
...
async def update_qc_input(record_id: UUID, data: dict, session: AsyncSession = Depends(get_db)):
...
async def list_qc(page: int = Query(1, ge=1), page_size: int = Query(50, ge=1, le=200),
    session: AsyncSession = Depends(get_db)):
    query = select(QcInspection).where(QcInspection.is_deleted == False).order_by(QcInspection.created_at.desc())
    count_q = select(func.count()).select_from(query.subquery())
    total = (await session.execute(count_q)).scalar_one()
...
async def update_qc(record_id: UUID, data: dict, session: AsyncSession = Depends(get_db)):
...
async def list_qc_items(qc_id: str, session: AsyncSession = Depends(get_db)):
    query = select(QcInspectionItem).where(QcInspectionItem.is_deleted == False, QcInspectionItem.inspection_id == qc_id)
    rows = await session.execute(query)
    return success_response([_clean_dict(r) for r in rows.scalars().all()])
...
async def create_qc_item(data: dict, session: AsyncSession = Depends(get_db)):
    ...
    session.add(item)
    await session.commit()
    return success_response({"id": str(item.id), "deviation": item.deviation, "is_blocked": item.is_blocked})
...
async def get_ba_records(session: AsyncSession = Depends(get_db)):
    ...
    records = (await session.execute(
        ...
        .where(ButylAcetateRecord.is_deleted == False)
