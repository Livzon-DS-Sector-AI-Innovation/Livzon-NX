...
class NCEService:
    ...
    async def list_records(self, page=1, page_size=20, workshop=None, event_type=None, date_from=None, date_to=None):
        return await self.repo.list(page=page, page_size=page_size, workshop=workshop, event_type=event_type, date_from=date_from, date_to=date_to)
    ...
    async def auto_link_batches(self, event):
        ...
        for bid in batch_ids:
            await self.session.execute(
                text("INSERT INTO production.nce_batch_links (id, nce_id, batch_id) VALUES (gen_random_uuid(), :nid, :bid)"),
    ...
    async def get_affected_batches(self, event_id: UUID):
        rows = await self.session.execute(text("""
            SELECT f.id, f.batch_no, f.product_name, f.fermenter, f.entry_date, f.discharge_date, f.status
        ...
        """), {"nid": event_id})
        return [{"id": str(r[0]), "batch_no": r[1], "product_name": r[2], "fermenter": r[3],
                 "entry_date": r[4].isoformat() if r[4] else None,
                 "discharge_date": r[5].isoformat() if r[5] else None, "status": r[6]} for r in rows]
