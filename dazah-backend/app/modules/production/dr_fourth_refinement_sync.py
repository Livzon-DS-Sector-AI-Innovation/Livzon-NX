...
REFINEMENT_SHEET = "6xzaQS"  # 四次精制台账
...
async def sync_dr_fourth_refinement(config: ProductionFeishuConfig, session: AsyncSession) -> dict:
    ...
    for ri, row in enumerate(rows):
        ...
        try:
            ...
            logger.warning(f"[DR四次精制同步] 行 {ri + DATA_START_ROW}: {e}")
            stats["errors"] += 1

    # 全量重建：先清空旧数据，再按飞书原始行序逐行插入（不做去重，忠实保留每一行）
    if data_rows:
        await session.execute(text("DELETE FROM production.dr_fourth_refinement"))
        for data in data_rows:
            cols = ", ".join(data.keys())
            vals = ", ".join(f":{k}" for k in data)
            await session.execute(
                text(
                    f"INSERT INTO production.dr_fourth_refinement "
    ...
    logger.info("[DR四次精制同步] 完成: %s", stats)
    return stats
