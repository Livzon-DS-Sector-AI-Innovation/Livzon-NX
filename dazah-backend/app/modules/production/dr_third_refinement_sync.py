...
REFINEMENT_SHEET = "5juozT"  # 三次精制台账
SHEET_RANGE = "A4:Y5000"  # 25 列
...
COL = {
    ...
    "product_pure_kg": 8,
    "yield_rate": 9,
    "mother_liquor_volume": 10,
    "mother_liquor_unit": 11,
    "mother_liquor_product_kg": 12,
    "impurity_6": 13,
    "impurity_1": 14,
    "impurity_2": 15,
    "impurity_7": 16,
...
async def sync_dr_third_refinement(config: ProductionFeishuConfig, session: AsyncSession) -> dict:
    app_secret = decrypt_secret(config.encrypted_app_secret)
    token = await _get_token(config.app_id, app_secret)
    logger.info("[DR三次精制同步] 读取飞书表格...")
    ...
    for ri, row in enumerate(rows):
        ...
        try:
            ...
            logger.warning(f"[DR三次精制同步] 行 {ri + DATA_START_ROW}: {e}")
            stats["errors"] += 1

    # 全量重建：先清空旧数据，再按飞书原始行序逐行插入（不做去重，忠实保留每一行）
    if data_rows:
        await session.execute(text("DELETE FROM production.dr_third_refinement"))
        for data in data_rows:
            cols = ", ".join(data.keys())
            vals = ", ".join(f":{k}" for k in data)
            await session.execute(
                text(
                    f"INSERT INTO production.dr_third_refinement "
    ...
    logger.info("[DR三次精制同步] 完成: %s", stats)
    return stats
