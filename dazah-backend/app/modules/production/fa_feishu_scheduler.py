...
async def _get_fa_spreadsheet_config(session: AsyncSession) -> dict:
    ...
    if not config:
        raise RuntimeError(
            ...
            f"sync_target={FA_CONFIG_SYNC_TARGET}），请在 203 车间页面点击同步设置进行配置"  # noqa: E501
        )
    return {
...
async def _sync_simple(
    ...
    for row in rows:
        ...
        if not in_data:
            if (
                (date_fmt == "slash" and re.match(r"\d{4}/\d+/\d+", c0))
                or (
                    date_fmt == "excel"
                    and (c0.isdigit() or re.search(r"\d+年\d+月\d+日", c0))
                )
                or (date_fmt == "dot" and re.match(r"\d{4}\.\d+\.\d+", c0))
                or (date_fmt == "year_month_day" and re.search(r"\d+年\d+月\d+日", c0))
            ):
                in_data = True
        ...
        if re.match(r"\d+月$|26年\d+月$", c0):
            continue
        if re.search(r"年.*月平均值|年年平均值|台账", " ".join(row)):
            continue
        if date_fmt == "excel" and (
            not c0 or (not c0.isdigit() and not re.search(r"\d+年\d+月\d+日", c0))
        ):
            continue  # 跳过非数据行
        if date_fmt == "excel" and c0 and c0.isdigit():
            cur_date = c0
        if date_fmt == "excel" and c0 and re.search(r"\d+年\d+月\d+日", c0):
            cur_date = c0  # 后期切换为中文日期
    ...
    for r in records:
        ...
        await session.execute(
            text(
                f'INSERT INTO production.{table} ("{'", "'.join(names)}") VALUES ({vals})'  # noqa: E501
            )
        )
...
async def _sync_acidification(session: AsyncSession, cfg: dict):
    ...
    for row in rows:
        ...
        if re.search(r"年.*月平均值|年年平均值", " ".join(row)):
            continue
        if not c0 and not c1 and not any(_g(row, i) for i in range(2, 31)):
            continue
    ...
    for r in records:
        ...
        await session.execute(
            text(
                f'INSERT INTO production.fa_acidification_records ("{'", "'.join(names)}") VALUES ({vals})'  # noqa: E501
            )
        )
    await session.commit()
    return {"rows": len(records)}
            ...
            for name, idx in cols:
                ...
                if name == "日期":
                    ...
                    if cur_date:
                        ...
                        if date_fmt == "excel":
                            d = _ed(v) if c0.isdigit() else _pd(v)
                        elif date_fmt == "year_month_day":
                            d = _pd(v)
                        else:
                            d = _pd(v)
                    ...
                    rec[name] = (
                        str(round(int(m.group(1)) + int(m.group(2)) / 60, 2))
                        if m
                        else (_n(v) if v else "NULL")
                    )
                elif name in (
                    "批号",
                    "备注",
    ...
    for m in modules:
        try:
            if m == "fermentation":
                ...
                results[m] = await _sync_simple(
                    session, cfg, sheet, table, cols, simple_parser(cols, fmt), fmt
                )
...
async def _fa_scheduled_job():
    ...
    if errors:
        logger.warning("[FA飞书同步] %d 模块失败: %s", len(errors), errors)
...
def stop_fa_sync_scheduler():
    ...
    if _fa_scheduler is not None:
        try:
            _fa_scheduler.shutdown(wait=False)
        except Exception:
            pass
        _fa_scheduler = None
