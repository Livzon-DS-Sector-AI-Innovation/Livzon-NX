...
from app.core.response import success_response  # noqa: E402
from app.core.secrets import decrypt_secret, encrypt_secret  # noqa: E402
from app.modules.production.production_feishu_client import (  # noqa: E402
    ProductionFeishuClient,  # noqa: E402
)
from app.modules.production.production_feishu_models import (  # noqa: E402
    ProductionFeishuConfig,  # noqa: E402
)
from app.modules.production.production_plan_service import (  # noqa: E402
...
class ProductionFeishuConfigUpsert(BaseModel):
    name: str = "生产飞书配置"
    product_name: str = ""
    app_id: str = ""
    app_secret: str = ""
...
async def test_feishu_config(
    ...
    try:
        ...
        steps.append(
            {"name": "获取 tenant_access_token", "status": "error", "message": str(e)}
        )
                            ...
                            steps.append(
                                {
                                    "name": "读取电子表格数据",
                                    "status": "ok",
                                    "message": f"电子表格模式 — 已读取 {len(values)} 行数据",  # noqa: E501
                                }
                            )
                            is_spreadsheet = True
                        else:
                            steps.append(
                                {
                                    "name": "读取电子表格数据",
                                    "status": "error",
                                    "message": f"飞书返回错误 (code={data.get('code')}): {data.get('msg')}",  # noqa: E501
                                }
                            )
                    else:
                        steps.append(
                            {
                                "name": "读取电子表格数据",
                                "status": "error",
                                "message": f"HTTP {resp.status_code}: {resp.text[:200]}",  # noqa: E501
                            }
                        )
    ...
    if not is_spreadsheet and body.table_id:
        try:
            ...
            steps.append(
                {"name": "读取多维表格记录", "status": "error", "message": str(e)}
            )

    ok = all(s["status"] != "error" for s in steps)
    return success_response(
        {"ok": ok, "steps": steps}, message="测试通过" if ok else "测试未通过"
    )
...
async def refresh_feishu_tables(session: AsyncSession = Depends(get_db)) -> Any:
    ...
    for c in configs:
        try:
            app_secret = (
                decrypt_secret(c.encrypted_app_secret) if c.encrypted_app_secret else ""
            )
            client = ProductionFeishuClient(
                app_id=c.app_id, app_secret=app_secret, app_token=c.bitable_app_token
            )
...
async def get_table_data(
    ...
    try:
        ...
        rows = await session.execute(
            text(
                f"SELECT * FROM production.{tn} ORDER BY created_at DESC LIMIT :l OFFSET :o"  # noqa: E501
            ),
        ...
        return success_response(
            items, meta={"page": page, "page_size": page_size, "total": total}
        )
    except Exception as e:
...
async def get_table_fields(
    ...
    fields = [
        {"name": info["name"], "db_column": info["db_column"]}
        for info in config.field_mapping.values()
    ]
    ...
    return success_response(
        {
            ...
            "sync_status": None,
            "sync_error": None,
            "last_synced_at": config.updated_at.isoformat()
        ...
        return success_response(
            {
                "table": {
                    ...
                    "is_enabled": config.is_active,
                    "field_count": 0,
                    "record_count": summary.get("created", 0)
                    + summary.get("updated", 0),
                    "sync_status": "success",
                    "sync_error": None,
                    "last_synced_at": updated.updated_at.isoformat()
