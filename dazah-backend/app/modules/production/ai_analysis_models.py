...
class AiAnalysis(BaseModel):
    ...
    batch_no: Mapped[str] = mapped_column(
        String(30), nullable=False, comment="目标批号"
    )
    ...
    llm_prompt: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="发给LLM的完整prompt"
    )
    llm_response: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="LLM原始返回"
    )
    ...
    severity: Mapped[str | None] = mapped_column(
        String(10), nullable=True, comment="严重程度 low/medium/high"
    )
    ...
    session_id: Mapped[str | None] = mapped_column(
        String(36), nullable=True, comment="会话ID，同会话共享"
    )
    message_seq: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False, comment="消息序号，0=首条报告"
    )
