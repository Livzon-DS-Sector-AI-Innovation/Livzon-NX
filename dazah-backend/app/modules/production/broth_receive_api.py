...
from app.modules.production.broth_receive_schemas import (
    BrothReceiveCreate,
    BrothReceiveResponse,
    BrothReceiveUpdate,
)
...
async def list_broth_receives(
    ...
    query = query.order_by(
        BrothReceive.seq_no.asc().nulls_last(),
        BrothReceive.receive_time.desc().nulls_last(),
    )
...
async def create_broth_receive(
    ...
    return success_response(
        BrothReceiveResponse.model_validate(record), message="创建成功"
    )
...
async def update_broth_receive(
    ...
    await session.flush()
    return success_response(
        BrothReceiveResponse.model_validate(record), message="更新成功"
    )
