...
SHEET = "1ijZSR"
COLS = [
    ("日期",0),("批号",1),("发酵液体积（kl)",2),("发酵液含量（g/L）",3),
    ("发酵液罐产（kg）",4),("用酸量（95-98%浓硫酸）",5),("PH（酸化后）",6),
    ("酸化液体积（kl)",7),("理论酸化液含量（g/L）",8),("PH",9),
    ("膜滤液体积（KL）",10),("膜滤液含量（g/L）",11),("膜滤液产品量（kg）",12),
...
PCT_COLS = {"批收率","渣损失率（渣苯丙量/罐产）","渣体积/发酵液体积","酸化液/发酵液体积","滤液体积/发酵液体积","平衡率"}
...
async def _read() -> list[list]:
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=60) as c:
        r = await c.get(f"/sheets/v2/spreadsheets/{SPREADSHEET}/values/{SHEET}",
...
def _pd(raw: str) -> str|None:
    ...
    return f"{2025 if int(m.group(1))==12 else 2026}-{int(m.group(1)):02d}-{int(m.group(2)):02d}"
...
def _p(v: str) -> str:
    ...
    try:
        n = float(v)
        if 0 < n < 1: return f"'{round(n*100)}%'"
        if n > 1: return f"'{round(n)}%'"
        return f"'{v}'"
    except: return f"'{v}'"
...
async def run(session: AsyncSession):
    ...
    for row in rows:
        ...
        if not c0 and not c1 and not _g(row,2): continue
        ...
        for name, idx in COLS:
            v = _g(row, idx)
            if name in PCT_COLS: rec[name] = _p(v) if v else "NULL"
    ...
    for r in records:
        ...
        await session.execute(text(f'INSERT INTO production.fa_acidification_records ("{'", "'.join(col_names)}") VALUES ({vals})'))
