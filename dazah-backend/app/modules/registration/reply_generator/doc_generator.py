...
def _set_run_font(
    run: Any, size: Any = 12, bold: Any = False, color: Any = None
) -> Any:
    """设置字体"""
...
def _add_cover_page(doc: Any, drug_info: dict[str, Any]) -> Any:
    """生成封面页"""
    for _ in range(6):
        doc.add_paragraph()
    ...
    _add_para(
        doc,
        f"资料类型：{drug_info.get('doc_type', '补充资料')}",
        size=16,
        align=WD_ALIGN_PARAGRAPH.LEFT,
    )
    _add_para(
        doc,
        f"产品名称：{drug_info.get('drug_name', '')}",
        size=16,
        align=WD_ALIGN_PARAGRAPH.LEFT,
    )
    _add_para(
        doc,
        f"登 记 号：{drug_info.get('registration_number', '')}",
        size=16,
        align=WD_ALIGN_PARAGRAPH.LEFT,
    )
    _add_para(
        doc,
        f"受 理 号：{drug_info.get('acceptance_number', '')}",
        size=16,
        align=WD_ALIGN_PARAGRAPH.LEFT,
    )
    _add_para(
        doc,
        f"申请人：{drug_info.get('company_name', '')}",
        size=16,
        align=WD_ALIGN_PARAGRAPH.LEFT,
    )
    _add_para(
        doc,
        f"申请事项：{drug_info.get('application_type', '首次登记')}",
        size=16,
        align=WD_ALIGN_PARAGRAPH.LEFT,
    )
    _add_para(
    ...
    _add_para(
        doc,
        f"联系电话：{drug_info.get('phone', '0756-8686208')}",
        size=16,
        align=WD_ALIGN_PARAGRAPH.LEFT,
    )

    doc.add_paragraph()
    address = drug_info.get("address", "中国，广东省，珠海市，珠海保税区联峰路22号")
    _add_para(
        doc,
        f"注册地址：{address}",
        size=14,
    ...
    _add_para(
        doc,
        f"关联制剂受理号：{drug_info.get('related_no', '/')}",
        size=16,
        align=WD_ALIGN_PARAGRAPH.LEFT,
    )
    _add_para(
...
def _add_toc_page(doc: Any, questions: list[dict[str, Any]]) -> Any:
    """生成目录页"""
...
def _add_body_content(doc: Any, questions: list[dict[str, Any]]) -> Any:
    """生成正文内容"""
    for q in questions:
        if q["level"] == 1:
            _add_para(doc, q["title"], size=12, bold=True)
        ...
        # 问题原文（蓝色加粗）
        _add_para(doc, q["title"], size=12, bold=True, color=RGBColor(0, 0, 255))
        doc.add_paragraph()
