...
class FieldMapping(Base):
    ...
    extraction_prompt: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="AI 提取提示：告诉 LLM 如何从素材中识别此字段的值"
    )
    source_type: Mapped[str] = mapped_column(
        String(50),
        nullable=False,
        default="asset_extract",
        comment=(
            "值来源类型: asset_extract(AI从素材提取)/asset_image(素材转图片)/"
            "fixed(固定值)/manual(手动)"
        ),
    )
    source_category: Mapped[str | None] = mapped_column(
        ...
        nullable=True,
        comment=(
            "素材分类名：指向 AssetCategory.category_name，限定 AI 只在对应分类的"
            "素材中查找"
        ),
    )
...
class AssetCategory(Base):
    ...
    appendix_slot: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="对应模板中的附录编号，如 附录1"
    )
