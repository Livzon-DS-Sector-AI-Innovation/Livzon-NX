...
class FieldFillService:
    ...
    async def _fill_single_field(
        ...
        if mapping.source_type == "manual":
            # 根据提取规则确定填充值
            fill_value = "详见附录"
            extraction_rule = getattr(mapping, "extraction_rule", None)
            if extraction_rule:
                # 尝试从提取规则中获取具体值
                if "详见附录4" in extraction_rule:
                    fill_value = "详见附录4"
                elif "详见附录5" in extraction_rule:
                    fill_value = "详见附录5"
        ...
        if mapping.source_type == "asset_file":
            # 查找匹配的素材
            source_pattern = getattr(mapping, "source_pattern", None)
            matched_asset = self._find_matching_asset(assets, source_pattern)
    ...
    async def _fill_to_document(
        self, doc: Any, mapping: FieldMapping, value: str
    ) -> bool:
        """将值填充到文档"""

        if mapping.location_type == "paragraph":
            return self._fill_paragraph(doc, mapping.location_hint or "", value)
        elif mapping.location_type == "table":
            if mapping.field_type == "table":
                # 填充完整表格
                return self._fill_complete_table(
                    doc, mapping.location_hint or "", value
                )
            else:
                # 填充单个单元格
                return self._fill_table(doc, mapping.location_hint or "", value)
    ...
    def _fill_complete_table(self, doc: Any, hint: str, table_data_json: str) -> bool:
        """填充完整表格（从JSON解析表格数据）"""
        import json
    ...
    async def _insert_image(
        self, doc: Any, mapping: FieldMapping, source_asset_path: str
    ) -> bool:
    ...
    def _fill_paragraph(self, doc: Any, hint: str, value: str) -> bool:
        """填充段落"""
    ...
    def _fill_table(self, doc: Any, hint: str, value: str) -> bool:
        """填充表格"""
