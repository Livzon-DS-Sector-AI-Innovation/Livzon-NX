...
class DossierService:
    ...
    def __init__(self, db: AsyncSession):
        ...
        self.storage_root = (
            Path(self.settings.STORAGE_ROOT) / "registration" / "dossier-writer"
        )
    ...
    async def create_product_dossier(
        self, data: ProductDossierCreate
    ) -> ProductDossier:
        """创建品种资料"""
    ...
    async def get_product_dossier(self, dossier_id: UUID) -> ProductDossier | None:
        ...
        if dossier:
            ...
            setattr(dossier, "chapter_count", chapter_count)
        return dossier
    ...
    async def list_product_dossiers(
        ...
        # 填充章节数量
        for item in items:
            setattr(item, "chapter_count", await self.repo.count_chapters(item.id))
        return items, total
    ...
    async def save_template_file(
        ...
        # 保存文件（覆盖同名文件）
        source_dir = self._required_path(dossier.source_templates_path, "原始模板目录")
        ...
        try:
            ...
            for template in templates:
                ...
                if matched_code:
                    ...
                    if matched_chapter:
                        # 创建 working copy
                        await self._create_working_copy_for_chapter(
                            dossier, template, matched_chapter
                        )
                        matched_count += 1

            # 更新状态
            await self.repo.update_product_dossier(
                dossier_id, parse_status="parsed", status="active"
            )
            await self.db.commit()
            ...
            return {
                "success": True,
                "message": (
                    f"解析完成，共上传 {len(templates)} 个模板文件，"
    ...
    async def _parse_docx_template(
        ...
        working_dir = self._required_path(dossier.working_path, "工作目录")
    ...
    def _generate_working_filename(
        ...
        if dossier.template_original_product_name:
            name = name.replace(
                dossier.template_original_product_name, dossier.product_name
            )
    ...
    def _build_chapter_tree(
        self, chapters: list[DossierChapter]
        ...
        def build_node(chapter: DossierChapter) -> ChapterResponse:
            ...
            children = sorted(
                children, key=lambda c: _chapter_sort_key(c.chapter_code or "")
            )
        ...
        root_chapters = sorted(
            root_chapters, key=lambda c: _chapter_sort_key(c.chapter_code or "")
        )
    ...
    async def _suggest_category(self, chapter_code: str, filename: str) -> UUID | None:
        """根据文件名自动猜测素材分类"""
        ...
        for cat in categories:
            ...
            if cat.description:
                ...
                desc_words = [
                    w
                    for w in re.split(r'[\s，。、；：""' "（）(),;]+", cat.description)
                    if len(w) > 2
                ]
    ...
    async def upload_chapter_asset(
        ...
        # 保存文件
        assets_dir = self._required_path(dossier.assets_path, "素材目录") / str(
            chapter_id
        )
        ...
        # 自动猜测分类
        suggested_category_id = await self._suggest_category(
            chapter.chapter_code or "", filename
        )
    ...
    async def export_dossier(
        self, dossier_id: UUID, chapter_ids: list[UUID] | None = None
    ) -> dict[str, Any]:
        """导出品种资料"""
        dossier = await self.repo.get_product_dossier(dossier_id)
        if not dossier:
        ...
        working_dir = self._required_path(dossier.working_path, "工作目录")
        outputs_dir = self._required_path(dossier.outputs_path, "导出目录")
        ...
        if chapter_ids and len(chapter_ids) == 1:
            ...
            if matched_chapters:
                codes = sorted(
                    set(ch.chapter_code for ch in matched_chapters if ch.chapter_code)
                )
                code_range = (
                    f"{codes[0]}-{codes[-1]}"
                    if len(codes) > 1
                    else (codes[0] if codes else "export")
                )
                export_filename = f"{dossier.product_name}_{code_range}_申报资料.docx"
                export_path = outputs_dir / export_filename
                first_working_file = matched_chapters[0].working_file
                if first_working_file:
                    source_file = working_dir / first_working_file
                    if source_file.exists():
                        shutil.copy2(source_file, export_path)
                    else:
                        export_path = None
    ...
    async def _create_working_copy_for_chapter(
        ...
        # 生成 working copy 文件名
        chapter_code = chapter.chapter_code or "chapter"
        working_filename = f"{chapter_code.replace('.', '_')}_{source_path.name}"
        working_path = working_dir / working_filename
    ...
    async def get_chapter_preview(self, chapter_id: UUID) -> dict[str, Any]:
        ...
        working_path = (
            self._required_path(dossier.working_path, "工作目录") / chapter.working_file
        )
    ...
    async def init_chapter_ai_config(self, chapter_code: str) -> dict[str, Any]:
        ...
        # 只处理指定章节的配置
        field_seed = S6_FIELD_MAPPINGS
        category_seed = S6_ASSET_CATEGORIES
        field_configs = [c for c in field_seed if c.get("chapter_code") == chapter_code]
        category_configs = [
            c for c in category_seed if c.get("chapter_code") == chapter_code
        ]
        ...
        for config in field_configs:
            mapping_stmt = select(FieldMapping).where(
            ...
            mapping_existing = mapping_result.scalar_one_or_none()
        ...
        for config in category_configs:
            category_stmt = select(AssetCategory).where(
            ...
            category_existing = category_result.scalar_one_or_none()
        ...
        return {
            "success": True,
            "message": (
                f"{chapter_code} 配置初始化完成: 字段映射 {created_mappings} 新建/"
                f"{skipped_mappings} 跳过, 素材分类 {created_categories} 新建/"
                f"{skipped_categories} 跳过"
            ),
            "chapter_code": chapter_code,
