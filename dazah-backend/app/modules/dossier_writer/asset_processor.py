...
class AssetExtractor:
    ...
    def extract_text_from_pdf_ocr(
        file_path: Path, lang: str = "chi_sim+eng"
    ) -> dict[str, Any]:
    ...
    def extract_table_value(
        tables: list[dict[str, Any]], field_name: str
    ) -> str | None:
        """从表格中提取字段值"""
        for table in tables:
...
class ContentFiller:
    ...
    @staticmethod
    def fill_paragraph_text(doc: Any, keyword: str, value: str) -> bool:
        """在段落中查找关键词后的冒号位置并填充值"""
    ...
    @staticmethod
    def fill_table_cell(doc: Any, field_name: str, value: str) -> bool:
        """填充表格中指定字段的值"""
    ...
    def insert_image_at_placeholder(
        doc: Any, placeholder: str, image_path: Path
    ) -> bool:
        """在占位符位置插入图片"""
        for para in doc.paragraphs:
