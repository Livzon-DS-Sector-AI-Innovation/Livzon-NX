...
def classify_solvents(
    ...
    for solvent_entry in solvents:
        ...
        if llm_class and llm_class != "pending":
            # LLM already classified this solvent; only enrich it with
            # PDE/limit data from the database.
                ...
                classified.append(
                    {
                        ...
                        "amount": solvent_entry.get("amount"),
                        "warning": (
                            "Not found in ICH Q3C - requires toxicological"
                            " justification"
                        ),
                    }
                )
...
def main() -> None:
    ...
    print("Building solvent index...", file=sys.stderr)
    ...
    print("Classifying LLM-extracted solvents...", file=sys.stderr)
    ...
    print(
        f"  Total unique solvents: {analysis['total_unique_solvents']}", file=sys.stderr
    )
