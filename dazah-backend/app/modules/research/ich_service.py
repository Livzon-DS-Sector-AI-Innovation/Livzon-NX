        ...
        if symbol not in elements_found:
            ...
            if hardcoded:
                ...
                elements_found[symbol] = {
                    ...
                    "ctcl": None,
                    "notes": (
                        "该元素不在ICH Q3D范围内，未建立PDE。需要逐案进行毒理学论证。"
                    ),
                }
...
def generate_report(process_text: str, elements: dict[str, dict[str, Any]]) -> str:
    ...
    report.append("")
    report.append(
        "**注意：** 本报告中所有浓度限度（μg/g）均基于选项1计算得出。选项2a、"
        "2b和3不在本评估范围内。"
    )
    report.append("")
    report.append("---")
    ...
    for symbol, data in sorted_elements:
        ...
        report.append(
            f"| {symbol} | {q3d_class} | {intentionally} | {oral} | {parenteral} |"
            f" {inhalation} | {cutaneous} |"
        )
