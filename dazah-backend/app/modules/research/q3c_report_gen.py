...
def generate_q3c_report(analysis, flag_class1=True, ich_data_source=""):
    ...
    if flag_class1 and class_counts["class1"] > 0:
        ...
        lines.append("")
        lines.append(
            "**根据 ICH Q3C 第 3.1 节:** Class 1 溶剂不应使用，"
            "除非能提供**强有力的科学依据**,即使残留量低于限值。"
        )
        lines.append("")
    ...
    if routine_release:
        for item in routine_release:
            lines.append(
                f"| {item['solvent']} | {item['class']} | {item['limit']} |"
                f" {item['reason']} |"
            )
    else:
    ...
    for solvent_name, data in sorted_solvents:
        ...
        lines.append(
            f"| {solvent_display} | {data['class'].replace('class', 'Class ')} |"
            f" {method} |"
        )
    ...
    lines.append(
        "- **Class 3 溶剂:** 根据 ICH Q3C 第 3.5 节，LOD ≤0.5% 可接受，或使用已验证的"
        " GC 方法"
    )
    lines.append(
        "- **10% 标准:** 第 2.2 节中的溶剂可通过工艺开发阶段控制，"
        "提供代表性数据证明残留量持续 ≤10% ICH 限值，可豁免批批检验。根据 EMA/EDQM CEP"
        " 要求，代表性数据需来自 **6 批连续中试规模批次** 或 **3"
        " 批连续工业生产规模批次** (EMA Annex I, CPMP/QWP/450/03-Rev.1)"
    )
    lines.append("")
    ...
    if needs_consultation:
        ...
        lines.append(
            "- ICH Q3C(R9):"
            " https://database.ich.org/sites/default/files/ICH_Q3C-R9_Guideline_2024_2024_Approved.pdf"
        )
