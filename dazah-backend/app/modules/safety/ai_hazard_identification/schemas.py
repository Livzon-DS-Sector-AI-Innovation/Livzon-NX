...
class RectificationSuggestion(BaseModel):
    ...
    immediate: str = Field(..., description="立即措施（24小时内），应急处置/隔离/停用")
    short_term: str = Field(..., description="短期整改（1-7天），修复/更换/培训")
    long_term: str = Field(..., description="长期预防，修订制度/纳入巡检/建立台账")
