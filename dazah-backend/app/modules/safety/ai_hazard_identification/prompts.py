...
必须输出三层结构，每层必须输出完整的中文段落（非短语、非关键词），\
每层至少 2-3 条具体可执行的措施：

- **立即措施（immediate）**：应急处置、隔离、停用、警示标识设置、\
切断危险源。描述具体的操作步骤、使用的工具/器材、执行后的确认标准。\
必须是发现隐患后24小时内可执行的动作。
- **短期整改（short_term）**：修复、更换、补充、清理、培训、交底。\
明确责任岗位/人员、完成时限（精确到小时或工作日）、引用具体的标准规范编号。\
描述整改的具体内容、方法和验收标准。
- **长期预防（long_term）**：修订制度/规程（写出具体制度名称）、纳入定期巡检（\
写明检查频次和检查项）、建立台账/档案（写明台账名称和更新频率）、系统排查同类问题\
（写明排查范围和责任部门）。明确长效机制的执行主体和监督考核方式。
...
3. 部门规章/判定标准：《化工和危险化学品生产经营单位重大生产安全事故隐患判定标准\
（试行）》
4. 集团/公司内部制度
...

{
  ...
  "hazard_category": "equipment | hazardous_storage | emergency_mgmt |\
instrument_electrical | lightning_antistatic | occupational_health |\
violation_operation | six_s | label_signage | process_mgmt |\
contractor_defect | documentation | special_operation",
  "hazard_level": "general | serious | major",
  "rectification_suggestion": {
    "immediate": "立即措施——完整中文段落，描述24小时内可执行的具体应急处理步骤、\
使用工具、确认标准，≥50字",
    "short_term": "短期整改——完整中文段落，描述1-7天的具体修复措施、责任岗位、\
完成时限、验收标准，引用具体标准编号，≥50字",
    "long_term": "长期预防——完整中文段落，描述长效机制的改进措施，包含具体制度名称、\
检查频次、台账名称、考核方式，≥50字"
  },
  "major_hazard_basis": "隐患判定依据，引用具体法规标准条文。\
格式：[法规/标准名称]第X条：'条文内容'"
}"""
...
# 多模态模式模板（有图片时）
VISION_PROMPT_TEMPLATE = """请仔细观察以下现场拍摄的缺陷照片，结合隐患描述文本，\
进行专业的安全隐患识别分析。
...
FEWSHOT_EXAMPLES = [
    {
        ...
        "output": {
            "key_defect": (
                "现场防爆电箱一处备用引入口未使用防爆堵头封堵，箱体内部积尘严重，"
                "存在粉尘进入电箱引发短路或爆炸的风险"
            ),
            ...
            "rectification_suggestion": {
                "immediate": (
                    "立即停止该防爆电箱的供电，断开上级电源开关，并在电箱周围设置临时警示标识"
                    "防止人员误操作；由持证电工对该电箱未封堵的引入口加装符合"
                    "GB 3836.1-2010标准的防爆堵头，使用防爆吸尘器清理箱内积尘"
                    "（严禁使用压缩空气吹扫）"
                ),
                "short_term": (
                    "由电气工程师于24小时内对责任区域内所有防爆电箱进行逐台排查，重点检查引入口"
                    "封堵完好性、箱体腐蚀情况及接地连续性，建立排查清单并由车间主任签字确认，"
                    "3个工作日内完成整改"
                ),
                "long_term": (
                    "修订《防爆电气设备巡检规程》，将防爆电箱引入口封堵状态、箱体腐蚀情况、"
                    "接地电阻值纳入每周专项检查项；建立防爆设备全生命周期台账（含型号、安装位置、"
                    "防爆等级、上次检查日期），由安全员每月抽查一次并签字留档"
                ),
            },
            "major_hazard_basis": (
                "《化工和危险化学品生产经营单位重大生产安全事故隐患判定标准》第十条："
                "爆炸危险场所未按国家标准安装使用防爆电气设备；GB 3836.1-2010 第15章："
                "电气设备引入装置的密封要求，所有不装电缆的引入口应使用与防爆型式相适应的堵头封堵"
            ),
        },
    },
    {
        "input": (
            "作业人员在2.5m高的脚手架平台进行管道焊接作业，未佩戴安全带，"
            "平台上未设置安全绳挂点"
        ),
        "output": {
            "key_defect": (
                "作业人员在2.5m高的脚手架平台进行管道焊接作业，未佩戴安全带，且平台上未设置"
                "安全绳挂点，存在高处坠落风险"
            ),
            ...
            "rectification_suggestion": {
                "immediate": (
                    "立即停止该作业人员的高处作业，将其撤离至安全地面；由安全员现场监督其正确佩戴"
                    "全身式安全带（符合GB 6095标准），确认安全带挂钩固定在可靠锚点"
                    "（承重≥15kN）后方可恢复作业"
                ),
                "short_term": (
                    "当日内对当班全体高处作业人员进行安全带正确佩戴和挂点选择的专项实操培训，"
                    "每人实操考核合格后方可上岗；同时在脚手架平台上加装临时安全绳（钢丝绳直径≥8mm），"
                    "两端用膨胀螺栓固定"
                ),
                "long_term": (
                    "在车间所有高处作业区域统一设置固定式安全绳挂点装置（膨胀螺栓固定于钢结构、"
                    "承重≥15kN、间距≤6m），纳入每日班前安全检查确认项；修订《高处作业安全管理规定》，"
                    "明确安全带'高挂低用'、双钩交替使用的具体要求"
                ),
            },
            "major_hazard_basis": (
                "GB 30871-2022 第5.2条：高处作业人员应正确佩戴符合国家标准的安全带，"
                "安全带应高挂低用；《安全生产法》第四十五条：生产经营单位必须为从业人员提供"
                "符合国家标准或行业标准的劳动防护用品"
            ),
        },
    },
    {
        "input": (
            "车间南侧消防疏散通道堆放约30袋成品包装物料，通道有效宽度不足0.8m，"
            "应急疏散指示灯被遮挡"
        ),
        "output": {
            "key_defect": (
                "车间南侧消防疏散通道堆放约30袋成品包装物料，通道有效通行宽度不足0.8m，"
                "应急疏散指示灯被物料遮挡，紧急情况下人员无法快速疏散"
            ),
            ...
            "rectification_suggestion": {
                "immediate": (
                    "立即组织人员将通道上堆放的30袋成品包装物料转移至指定物料暂存区，清理通道上一切"
                    "障碍物，确保通道净宽≥1.4m且应急疏散指示灯完全无遮挡，"
                    "由当班班长现场确认后签字"
                ),
                "short_term": (
                    "24小时内在消防通道两侧地面施划黄色禁停标线（宽100mm，使用耐磨环氧地坪漆），"
                    "在通道墙面醒目位置张贴'消防通道 禁止堆放'反光警示标识（间距≤5m），"
                    "明确物料暂存区边界并设置标牌"
                ),
                "long_term": (
                    "修订《车间定置管理与消防通道管理规定》，明确消防疏散通道净宽≥1.4m的硬性指标及"
                    "违规堆放处罚条款；由安全员每月对全厂消防通道进行专项检查并拍照留档，检查结果纳入"
                    "部门月度安全绩效考核"
                ),
            },
            "major_hazard_basis": (
                "GB 50016-2014（2018年版）第7.3.1条：疏散通道的净宽度不应小于1.1m；"
                "《安全生产法》第四十二条：生产经营场所和员工宿舍应当设有符合紧急疏散要求、"
                "标志明显、保持畅通的出口和疏散通道"
            ),
        },
    },
    {
        "input": (
            "一级动火作业票（编号DH-2026-0612）中，现场监护人、动火负责人签章栏空白，"
            "审批时间与实际作业时间不一致"
        ),
        "output": {
            "key_defect": (
                "一级动火作业票（编号DH-2026-0612）审批流程不完整：现场监护人、动火负责人签章栏"
                "均为空白，且审批单上记录的审批时间晚于现场实际动火作业开始时间，存在无监管动火作业风险"
            ),
            ...
            "rectification_suggestion": {
                "immediate": (
                    "立即暂停该动火作业，撤走动火器具并清理动火点周围可燃物；要求现场监护人和动火负责人"
                    "到场在作业票上补签确认，对照GB 30871-2022逐项重新核查安全措施落实"
                    "情况，全部合格后"
                    "重新办理审批手续方可恢复作业"
                ),
                "short_term": (
                    "本周内组织涉及特殊作业审批的所有相关人员（动火负责人、监护人、审批人、"
                    "安全员）进行"
                    "GB 30871-2022专项培训，重点讲解作业票证规范填写要求、审批流程"
                    "及各岗位安全职责，"
                    "培训后书面考试，不合格者暂停审批权限"
                ),
                "long_term": (
                    "建立特殊作业票证三级审核制度（班组自检→车间复检→安全部抽检），每周对已归档票证按"
                    "10%比例随机抽查，重点核查签章完整性、时间逻辑一致性及安全措施落实情况；"
                    "检查结果纳入月度安全绩效考核，连续2个月不合格的车间取消自主审批权限"
                ),
            },
            "major_hazard_basis": (
                    "GB 30871-2022 第4.7条：特殊作业审批手续应齐全、安全措施应全部"
                    "落实、"
                    "作业环境应符合安全要求；《安全生产法》第四十六条：生产经营单位进行爆破、吊装、"
                    "动火、临时用电等"
                    "危险作业，应当安排专门人员进行现场安全管理"
            ),
        },
    },
]
...
def get_db_seed_config() -> dict[str, Any]:
    ...
    return {
        ...
        "workflow_description": (
            "基于缺陷图片+隐患描述，自动输出六大AI字段：隐患描述（AI）、隐患分类（AI）、隐患类别（AI）、"
            "隐患级别（AI）、整改建议（AI）、隐患判定依据（AI）"
        ),
            ...
            "scripts": [
                {
                    ...
                    "input_info": (
                        "隐患记录的全部字段（隐患编号、描述、部门、地点、缺陷图片）"
                    ),
                    "work_rules": WORK_RULES,
                    "reference_docs": (
                        "《安全生产法》《消防法》GB/T 13861-2022 GB 30871-2022 GB 3836 "
                        "GB 50016 GB 50160 《化工和危险化学品"
                        "生产经营单位重大生产安全事故隐患判定标准"
                        "（试行）》"
                    ),
                    "output_format": OUTPUT_FORMAT,
                },
                {
                    ...
                    "input_info": (
                        "步骤1的完整输出（隐患描述、分类、类别、级别、判定依据）"
                    ),
                    "work_rules": WORK_RULES,
                    "reference_docs": "同上",
                    "output_format": (
                        '{"corrective_preventive_measures": '
                        '"结合步骤1的所有发现，生成具体的整改措施文本"}'
                    ),
                },
            ]
        },
