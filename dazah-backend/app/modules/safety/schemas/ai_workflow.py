...
class AIWorkflowConfigBase(BaseModel):
    ...
    script_configs: list[dict[str, Any]] | dict[str, Any] | None = Field(
        None, description="脚本配置 JSON"
    )
...
class ReferenceAttachmentResponse(BaseModel):
    ...
    file_type: str | None = Field(
        None, description="文件类型: pdf / docx / xlsx / txt / md"
    )
    file_size: int | None = Field(None, description="文件大小（字节）")
    markdown_path: str | None = Field(
        None, description="Markdown 转换文件路径（AI 读取用）"
    )
    knowledge_id: str | None = Field(
        None, description="知识库文章 ID（type=knowledge 时）"
    )
