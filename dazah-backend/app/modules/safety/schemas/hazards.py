...
class RectificationReplyRequest(BaseModel):
    ...
    corrective_preventive_measures: str | None = Field(
        None, description="AI整改建议（兼容旧字段）"
    )
    rectification_reply: str | None = Field(None, description="整改回复内容（新字段）")
    actual_completion_date: datetime | None = Field(
        None, description="整改完成时间（默认当前时间）"
    )
...
class HazardReportBase(BaseModel):
    ...
    inspection_category: str | None = Field(
        None, max_length=64, description="检查类别（日常检查/专项检查…）"
    )
    hazard_type: HazardType = Field(..., description="隐患分类（人/物/环/管）")
    hazard_level: HazardLevel = Field(HazardLevel.GENERAL, description="隐患等级")
    hazard_category: HazardCategory | None = Field(
        None, description="隐患类别（设备设施/危化储存…）"
    )
    ...
    discovered_by_name: str | None = Field(
        None, max_length=100, description="检查人员姓名"
    )
    inspector_department: str | None = Field(
        None, max_length=500, description="检查人员部门（Bitable 多选，逗号分隔）"
    )
    ...
    rectification_responsible_person: uuid.UUID | None = Field(
        None, description="整改责任人（FK → identity.users）"
    )
    rectification_responsible_person_name: str | None = Field(
        None, max_length=100, description="整改责任人姓名"
    )
...
class HazardReportCreate(HazardReportBase):
    ...
    hazard_no: str | None = Field(  # type: ignore[assignment]  # create allows server-generated numbers
        None, max_length=64, description="隐患编号（留空自动生成）"
    )
    hazard_type: HazardType | None = Field(None, description="隐患分类")  # type: ignore[assignment]  # create fields are optional
    hazard_level: HazardLevel | None = Field(None, description="隐患等级")  # type: ignore[assignment]  # create fields are optional
    hazard_category: HazardCategory | None = Field(None, description="隐患类别")
    description: str | None = Field(None, description="隐患描述")  # type: ignore[assignment]  # create fields are optional
    discovered_at: datetime | None = Field(None, description="发现时间")  # type: ignore[assignment]  # create fields are optional
...
class HazardReportUpdate(BaseModel):
    ...
    discovered_by: uuid.UUID | None = Field(
        None, description="发现人（FK → identity.users）"
    )
    discovered_by_name: str | None = Field(
        None, max_length=100, description="检查人员姓名"
    )
    ...
    notes: str | None = Field(None, description="备注")
    # ── AI 流程字段 ──
    ai_node_progress: str | None = Field(
        None, max_length=50, description="AI流程节点进度"
    )
    overall_status: str | None = Field(None, max_length=20, description="整体状态")
    ai_error_message: str | None = Field(None, description="AI错误信息")
    script1_review_status: str | None = Field(
        None, max_length=20, description="AI隐患识别审核状态"
    )
    script2_review_status: str | None = Field(
        None, max_length=20, description="AI整改建议审核状态"
    )
