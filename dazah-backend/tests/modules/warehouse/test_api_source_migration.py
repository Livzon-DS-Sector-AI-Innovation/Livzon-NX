...
@pytest.mark.anyio
async def test_get_feishu_material_page_local_force_runs_full_sync(
    db_session: AsyncSession,
) -> None:
    """本地快照模式「同步最新数据」（force=1）执行全量同步，再读镜像。"""
    service = WarehouseService(db_session)
    sync_mock = AsyncMock()
    ...
    sync_mock.assert_awaited_once_with("raw-ledger", incremental=False)
...
@pytest.mark.anyio
async def test_get_feishu_material_page_local_incremental_runs_incremental_sync(
    db_session: AsyncSession,
) -> None:
    """本地快照模式「刷新」（incremental=1）执行增量同步，再读镜像。"""
    service = WarehouseService(db_session)
    sync_mock = AsyncMock()
    local_mock = AsyncMock(return_value={"source": "local_snapshot", "total": 5})
    with (
