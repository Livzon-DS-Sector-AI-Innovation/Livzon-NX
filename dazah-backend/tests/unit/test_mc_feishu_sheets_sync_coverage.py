...
class _BranchDB:
    ...
    def __init__(self, matchers: Any = None, raise_on: Any = None) -> None:
...
def _as_session(value: Any) -> AsyncSession:
...
def _install_httpx(handler: Any) -> Any:
    ...
    return patch.object(getattr(sync, "httpx"), "AsyncClient", fake_async_client)
...
async def test_sync_crude_full_path_with_skip() -> None:
    ...
    with patch.object(sync, "_read_sheet_range", new=AsyncMock(return_value=rows)):
        stats = await sync._sync_crude(_as_session(session), "tok", "app", "sec")
...
async def test_sync_crude_existing_subtank_skips_create() -> None:
    ...
    with patch.object(sync, "_read_sheet_range", new=AsyncMock(return_value=rows)):
        stats = await sync._sync_crude(_as_session(db), "tok", "app", "sec")
...
async def test_sync_crude_row_error_rolls_back() -> None:
    ...
    db = _BranchDB()
    setattr(db, "flush", AsyncMock(side_effect=RuntimeError("flush boom")))
    setattr(db, "rollback", AsyncMock(side_effect=RuntimeError("rollback boom")))
    with patch.object(sync, "_read_sheet_range", new=AsyncMock(return_value=rows)):
        stats = await sync._sync_crude(_as_session(db), "batch", "app", "sec")
...
async def test_sync_extraction_update_and_skip() -> None:
    ...
    with patch.object(sync, "_read_sheet_range", new=AsyncMock(return_value=rows)):
        stats = await sync._sync_extraction(_as_session(db), "tok", "app", "sec")
...
async def test_sync_refinement_update_and_skip() -> None:
    ...
    with patch.object(sync, "_read_sheet_range", new=AsyncMock(return_value=rows)):
        stats = await sync._sync_refinement(_as_session(db), "tok", "app", "sec")
    assert stats["updated_records"] == 1
    assert stats["updated_inputs"] == 2
    assert stats["skipped"] == 2
...
async def test_sync_blending_create_path() -> None:
    ...
    db = _BranchDB()
    with patch.object(sync, "_read_sheet_range", new=AsyncMock(return_value=rows)):
        stats = await sync._sync_blending(_as_session(db), "tok", "app", "sec")
...
async def test_sync_qc_create_and_skip() -> None:
    ...
    db = _BranchDB()
    with patch.object(sync, "_read_sheet_range", new=AsyncMock(return_value=rows)):
        stats = await sync._sync_qc(_as_session(db), "tok", "app", "sec")
    assert stats["created_records"] == 1
    assert stats["created_inputs"] == 2
    assert stats["skipped"] == 2
...
async def test_sync_ba_header_only_error() -> None:
    ...
    db = _BranchDB()
    with patch.object(sync, "_read_sheet_range", new=AsyncMock(return_value=rows)):
        stats = await sync._sync_ba(_as_session(db), "tok", "app", "sec")
    assert stats["errors"] == 1
...
async def test_scheduled_job_success_path() -> None:
    ...
    class SessionContext:
        ...
        async def __aexit__(self, *_args: Any) -> Any:
            return False
