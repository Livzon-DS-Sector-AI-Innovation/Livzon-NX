...
def upgrade() -> None:
    ...
    op.add_column(
        "production_plans",
        sa.Column(
            "product_name", sa.String(length=128), nullable=False, comment="产品"
        ),
        schema="production",
    )
    op.add_column(
        "production_plans",
        sa.Column("plan_date", sa.Date(), nullable=True, comment="日期"),
        schema="production",
    )
    op.add_column(
        "production_plans",
...
def downgrade() -> None:
    # 回滚: 删新字段
    op.drop_index(
        "ix_production_plans_date", table_name="production_plans", schema="production"
    )
    op.drop_index(
        "ix_production_plans_product",
