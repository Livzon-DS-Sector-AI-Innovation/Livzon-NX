...
def upgrade():
    ...
    op.create_table('non_conforming_events',
    sa.Column('event_time', sa.DateTime(timezone=True), nullable=False, comment='发生时间'),
    sa.Column('restore_time', sa.DateTime(timezone=True), nullable=True, comment='恢复正常时间'),
    ...
    schema='production')
    op.create_index('ix_nce_event_time', 'non_conforming_events', ['event_time'], unique=False, schema='production')
    op.create_index('ix_nce_workshop', 'non_conforming_events', ['workshop'], unique=False, schema='production')
    op.create_index('ix_nce_event_type', 'non_conforming_events', ['event_type'], unique=False, schema='production')
