...
def upgrade() -> None:
    op.add_column('shift_handovers', sa.Column('status', sa.String(length=16), nullable=True), schema='production')
    op.add_column('shift_handovers', sa.Column('confirmed_at', sa.DateTime(timezone=True), nullable=True), schema='production')
    op.execute("UPDATE production.shift_handovers SET status = 'pending' WHERE status IS NULL")
