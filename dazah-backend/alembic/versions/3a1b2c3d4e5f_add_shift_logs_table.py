...
down_revision: Union[str, None] = 'a92e3c435c3b'
...
def upgrade() -> None:
    ...
    sa.Column('shift', sa.String(length=16), nullable=False, comment='班次（morning/afternoon/night）'),
    ...
    sa.Column('production_summary', sa.Text(), nullable=True, comment='本班生产情况'),
    ...
    op.create_index('ix_shift_logs_date', 'shift_logs', ['log_date'], unique=False, schema='production')
    op.create_index('ix_shift_logs_workshop', 'shift_logs', ['workshop'], unique=False, schema='production')


def downgrade() -> None:
    op.drop_index('ix_shift_logs_workshop', table_name='shift_logs', schema='production')
