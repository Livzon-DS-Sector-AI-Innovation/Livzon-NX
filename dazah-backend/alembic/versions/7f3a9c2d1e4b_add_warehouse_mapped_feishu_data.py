...
def upgrade() -> None:
    ...
    op.add_column(
        "feishu_tables",
        sa.Column("active_mirror_version", sa.String(64), nullable=True),
        schema="warehouse",
    )
    op.add_column(
        "feishu_fields",
    ...
    op.add_column(
        "feishu_records",
        sa.Column("mirror_version", sa.String(64), nullable=True),
        schema="warehouse",
    )
    op.add_column(
        "feishu_records",
    ...
    op.create_index(
        "ix_warehouse_feishu_roots_active",
        "feishu_source_roots",
        ["config_id", "is_active"],
        schema="warehouse",
    )
    ...
    op.create_index(
        "ix_warehouse_page_bindings_page",
        "feishu_page_bindings",
        ["page_key", "is_enabled"],
        schema="warehouse",
    )
    ...
    op.create_index(
        "ix_warehouse_data_sync_runs_table",
        "feishu_data_sync_runs",
        ["table_pk", "started_at"],
        schema="warehouse",
    )
    ...
    op.create_index(
        "ix_warehouse_record_snapshots_table",
        "feishu_record_snapshots",
        ["table_pk", "captured_at"],
        schema="warehouse",
    )
    ...
    op.create_index(
        "ix_warehouse_analysis_profiles_active",
        "feishu_analysis_profiles",
        ["is_active", "auto_run"],
        schema="warehouse",
    )
    ...
    op.create_table(
        ...
        *_base_columns(),
        sa.UniqueConstraint(
            "profile_id", "version", name="uq_warehouse_prompt_version"
        ),
        schema="warehouse",
    )
    ...
    op.create_index(
        "ix_warehouse_analysis_runs_profile",
        "feishu_analysis_runs",
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_warehouse_analysis_runs_profile",
        table_name="feishu_analysis_runs",
        schema="warehouse",
    )
