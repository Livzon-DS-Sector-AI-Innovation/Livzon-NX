...
async def optimize(
    ...
    - **objective_modes**: Comma-separated list of "max" or "min" for each
      objective (default: "max")
    ...
    try:
        ...
        if not objectives_list or objectives_list == [""]:
            raise HTTPException(
                status_code=400, detail="objectives list cannot be empty"
            )
        ...
        if len(objectives_list) != len(modes_list):
            raise HTTPException(
                status_code=400,
                detail=(
                    f"objectives ({len(objectives_list)}) and objective_modes "
                    f"({len(modes_list)}) must have the same length"
                ),
            )
        ...
        response = OptimizeResponse(csv_data=csv_data, row_count=row_count)
        ...
        raise HTTPException(
            status_code=500, detail=f"EDBO+ optimization failed: {str(e)}"
        )
    finally:
