...
for batch in [1, 2, 3, 5]:
    for acq_i in ["EHVI"]:
        for sampling_method in ["seed", "lhs", "cvtsampling"]:
            ...
            columns_regression = columns_regression.drop(
                [sort_column, "yield", "cost"]
            ).tolist()
            ...
            label_benchmark = (
                f"benchmark_dft_acq_{acq}_batch_{batch}_seed_{seed}_"
                f"init_sampling_{sampling_method}.csv"
            )
            ...
            if not os.path.exists(f"./results/{label_benchmark}"):
                ...
                shutil.move(
                    f"pred_{label_benchmark}", f"results/pred_{label_benchmark}"
                )
                shutil.move(
                    f"results_{label_benchmark}", f"results/results_{label_benchmark}"
                )
