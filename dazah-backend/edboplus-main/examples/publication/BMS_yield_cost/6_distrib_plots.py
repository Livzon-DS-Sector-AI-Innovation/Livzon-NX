...
df_0 = pd.read_csv(
    f"./results/results_benchmark_dft_acq_EHVI_batch_3_seed_1_init_sampling_{samplings[0]}.csv"
)
df_0["step"] += 1
df_0 = df_0[df_0["n_experiments"] < max_num_experiments]

df_1 = pd.read_csv(
    f"./results/results_benchmark_dft_acq_EHVI_batch_3_seed_1_init_sampling_{samplings[1]}.csv"
)
df_1["step"] += 1
df_1 = df_1[df_1["n_experiments"] < max_num_experiments]

df_2 = pd.read_csv(
    f"./results/results_benchmark_dft_acq_EHVI_batch_3_seed_1_init_sampling_{samplings[2]}.csv"
)
df_2["step"] += 1
...
for i in range(0, 3):
    ...
    plt.savefig(
        f"./results_plots/subplot_{samplings[i]}_{objective_1}.svg",
        format="svg",
        dpi=500,
    )
    plt.show()
    ...
    plt.savefig(
        f"./results_plots/subplot_{samplings[i]}_{objective_2}.svg",
        format="svg",
        dpi=500,
    )
    plt.show()
