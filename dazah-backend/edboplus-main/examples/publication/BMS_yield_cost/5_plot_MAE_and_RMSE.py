...
def get_pareto_points(objective_values):
    ...
    pareto_ground = pareto.eps_sort(
        tables=objective_values, objectives=np.arange(2), maximize_all=True
    )
...
df_all = pd.read_csv(
    f"./results/results_benchmark_dft_acq_EHVI_batch_{batch_sizes[0]}_seed_1_init_sampling_{samplings[0]}.csv"
)
for i in batch_sizes:
    for j in samplings:
        df_i = pd.read_csv(
            f"./results/results_benchmark_dft_acq_EHVI_batch_{i}_seed_1_init_sampling_{j}.csv"
        )
...
df_finish = df_all[
    (df_all["n_experiments"] < max_number_experiments + 2)
    & (df_all["n_experiments"] > max_number_experiments - 2)
]
