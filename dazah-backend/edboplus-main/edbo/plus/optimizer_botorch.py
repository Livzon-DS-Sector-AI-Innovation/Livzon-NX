...
class EDBOplus:
    ...
    def generate_reaction_scope(
        ...
        print(
            f"The scope was generated and contains {n_combinations} possible reactions!"
        )
        return df
    ...
    def _init_sampling(df, batch, sampling_method, seed):
        ...
        if len(ohe_columns) > 0:
            print(
                f"The following columns are categorical and will be encoded"
                f" using One-Hot-Encoding: {ohe_columns}"
            )
        # Encode OHE.
        df_sampling = pd.get_dummies(
            df,
            prefix=ohe_columns,
            columns=ohe_columns,
            drop_first=True,
        ...
        with HiddenPrints():
            ...
            if sampling_method == "random":
                ...
                idaes = LatinHypercubeSampling(
                    df_sampling, batch, sampling_type="selection"
                )
            ...
            # Sometimes LHS or CVT returns fewer samples than requested. Add
            # random samples in that case.
            # IMPORTANT: Sample from df_sampling (encoded), not df (original),
            # to match dimensions.
            additional_samples = None
            if len(samples) < batch:
                additional_samples = df_sampling.sample(
                    n=batch - len(samples), random_state=seed, replace=True
                )
                additional_samples = additional_samples.reset_index(drop=True)
            # Add additional samples to the dataframe. If duplicates occur,
            # generate new samples until the batch size is reached.
            extra_seed = 1
            while len(samples) < batch:
                samples = pd.concat([samples, additional_samples]).drop_duplicates(
                    ignore_index=True
                )
                additional_samples = df_sampling.sample(
                    n=batch - len(samples), random_state=seed + extra_seed, replace=True
                )
                extra_seed += 1
        ...
        print(
            f"Generated {len(samples)} initial samples using "
            f"{sampling_method} sampling (seed = {seed}). Run finished!"
        )
    ...
    def run(
        ...
        columns_features: list
            List containing the names of the columns to be included in the
            regression model. By default set to 'all', which means the
            algorithm will automatically select all columns that are not in
            the *objectives* list.

        batch: int
            Number of experiments that you want to run in parallel. For
            instance *batch = 5* means that you will run 5 experiments in each
            EDBO+ run. You can change this number at any stage of the
            optimization, so don't worry if you change your mind after
            creating or initializing the reaction scope.

        get_predictions: boolean
            If True it will print out a *csv file* with the predictions.
            You can also access *predicted_mean* and *predicted_variance*
            through the EDBOPlus class.

        directory: string
            name of the directory to save the results of the optimization.

        filename: string
            Name of the file to save a *csv* with the priority list. If
            *get_predictions=True*, EDBO+ will automatically save a second file
            including the predictions (*pred_filename.csv*).

        init_sampling_method: string:
            Method for selecting the first samples in the scope (in absence).
            Choices are:
        ...
        if not isinstance(objective_mode, list):
        ...
        if len(obj_in_df) == 0:
            print(
                "There are no experimental observations yet. "
                "Random samples will be drawn."
            )
            df = self._init_sampling(
                df=df, batch=batch, seed=seed, sampling_method=init_sampling_method
            )
            ...
            original_df = original_df.loc[
                :, ~original_df.columns.str.contains("^Unnamed")
            ]
        ...
        if (
            columns_features == "all"
        ):  # replacing with actual list of all features for printout
            columns_features = list(
                set(df.columns.tolist()) - set(objectives) - set(["priority"])
            )
        ...
        idx_test = (
            data[
                data.apply(
                    lambda r: r.astype(str).str.contains("PENDING", case=False).any(),
                    axis=1,
                )
            ]
        ).index.values
        ...
        if len(df_train_x.values) == 0:
            msg = (
                "The scope was already generated, please "
    ...
    def _model_run(
        ...
        if self.acquisition_sampler == "IIDNormalSampler":
            sampler = IIDNormalSampler(
                num_samples=sobol_num_samples, collapse_batch_dims=True, seed=seed
            )
        ...
        if acquisition_function.lower() == "ehvi":
            ...
            ehvi_acquisition = qExpectedHypervolumeImprovement(
            ...
            acq_result = optimize_acqf_discrete(
                acq_function=ehvi_acquisition, choices=test_x, q=batch, unique=True
            )
        ...
        if acquisition_function.lower() == "noisyehvi":
            with warnings.catch_warnings():
                ...
                if (
                    n_objectives > 1
                ):  # NoisyEHVI fails for one objective, so use EI instead.
                    surrogate_model = ModelListGP(*individual_models)
                    train_x = torch.tensor(init_train).to(**tkwargs).double()
                    ...
                    acq_fct = qExpectedImprovement(
                        model=surrogate_model, best_f=best_value, sampler=sampler
                    )

                acq_result = optimize_acqf_discrete(
                    acq_function=acq_fct, choices=test_x, q=batch, unique=True
                )
        ...
        for i in range(0, len(data.values), n_chunks):
            ...
            self.predicted_mean[i : i + n_chunks] = scaler_y.inverse_transform(
                preds.mean.detach().cpu().numpy()
            )
            self.predicted_variance[i : i + n_chunks] = scaler_y.inverse_transform_var(
                preds.variance.detach().cpu().numpy()
            )
    ...
    def expected_improvement(self, train_y, mean, variance, maximizing=False):
        ...
        ----------
            ...
            train_y: Numpy array.
                Numpy array containing previously observed train targets.
            maximizing: Boolean.
                Boolean flag indicating whether the loss function is to be
        ...
        with np.errstate(divide="ignore"):
            z_score = scaling_factor * (mean - loss_optimum) / sigma
            expected_improvement = scaling_factor * (mean - loss_optimum) * norm.cdf(
                z_score
