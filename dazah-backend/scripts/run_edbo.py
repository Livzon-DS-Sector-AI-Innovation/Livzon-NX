...
def main() -> Any:
    if len(sys.argv) != 6:
        print(
            json.dumps(
                {
                    "error": "Usage: run_edbo.py <csv_path> <objectives_json> "
                    "<modes_json> <batch_size> <save_prediction>"
                }
            )
        )
        sys.exit(1)
    ...
    try:
        optimizer_module = importlib.import_module("edbo.plus.optimizer_botorch")
        edbo_plus_cls = getattr(optimizer_module, "EDBOplus")
        ...
        # Output result
        output = {"csv_data": csv_b64, "row_count": len(result_df)}
