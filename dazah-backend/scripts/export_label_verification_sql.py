...
python3 scripts/export_label_verification_sql.py
...
async def export_label_verification_sql() -> Any:
    async with engine.begin() as conn:
        result = await conn.execute(
            text(
                "SELECT * FROM production.label_verifications WHERE is_deleted = false "
