...
Example:
    ...
    python scripts/import_quality_data.py /tmp/converted.zip \
        --user-mapping /tmp/user_mapping.json
"""
...
def parse_json_field(value: str | None) -> dict[Any, Any] | list[Any] | None:
    """Parse JSON field, return None if empty."""
    if not value or value in ("{}", "[]", ""):
        return None
    try:
        parsed = json.loads(value)
        return cast(dict[Any, Any] | list[Any], parsed)
...
async def import_weekly_confirmations(
    ...
    """Import department_weekly_confirmation.csv to
    quality.department_weekly_confirmations.
    """
    reader = csv.DictReader(io.StringIO(csv_data))
...
async def main() -> Any:
    ...
    parser.add_argument(
        "--db-url",
        default="postgresql://erp_user:LivzonSyntpharm@postgres:5432/erp",
        help="Database URL",
    )
