...
def probe() -> Any:
    ...
    for i in range(15):
        ...
        print(
            f"   [{(i + 1) * 2}s] title='{title[:40]}' "
            f"cookies={len(cookies)} content={content_len}"
        )
    ...
    if waf_passed:
        ...
        if not report["api_found"]:
            ...
            try:
                ...
                print(
                    f"   Fetch: status={fetch_result.get('status')} "
                    f"is_json={fetch_result.get('is_json')}"
                )
    ...
    if report["api_found"]:
        ...
        try:
            p2 = page.evaluate("""async () => {
                try {
                    const resp = await fetch(
                        '/zdyz/getDomesticGuideList?pageNum=2&pageSize=10', {
            ...
            if p2.get("is_json") and p2.get("data"):
                d2 = p2["data"]
                print(
                    f"   ✅ 第二页! current={d2.get('current')} "
                    f"records={len(d2.get('records', []))}"
                )
    ...
    if report.get("api_data") and report["api_data"].get("first_record"):
        ...
        if zdyz_id:
            ...
            try:
                dr = page.evaluate(f"""async () => {{
                    try {{
                        ...
                        return {{
                            ...
                            url: resp.url,
                            body_length: text.length,
                            is_html: text.includes('<html') ||
                                text.includes('<!DOCTYPE'),
                            title_match: text.includes('指导原则') ||
                                text.includes('指南'),
                ...
                print(
                    f"   详情页: status={dr.get('status')} "
                    f"length={dr.get('body_length')} is_html={dr.get('is_html')}"
                )
...
def main() -> Any:
    ...
    if report["api_data"]:
        ...
        if ad.get("is_json"):
            ...
            if "first_record" in ad:
                print(
                    f"    首条: "
                    f"{json.dumps(ad['first_record'], ensure_ascii=False)[:300]}"
                )
    ...
    if report["pagination_data"]:
        ...
        if pd.get("page2_first_record"):
            print(
                f"    Page2 首条: "
                f"{json.dumps(pd['page2_first_record'], ensure_ascii=False)[:200]}"
            )
