...
S6_ASSET_CATEGORIES: list[dict[str, Any]] = [
    {
    ...
    },
    {
        ...
        "appendix_slot": None,
        "description": (
            "包装材料（如药用铝瓶）的企业质量标准文档，包含包材类型、执行标准号、"
            "检验项目表等"
        ),
        "sort_order": 2,
    },
    {
        ...
        "appendix_slot": None,
        "description": (
            "包材生产商授权给药品生产企业的授权书，包含生产商名称、登记号，"
            "同时需转为图片插入正文"
        ),
        "sort_order": 3,
    },
    {
        ...
        "appendix_slot": None,
        "description": (
            "可能包含营业执照、CDE公示、检验报告单等多种内容的合并文档（PDF），"
            "AI 会按页拆分识别"
        ),
        "sort_order": 4,
    },
    ...
    },
    {
        ...
        "appendix_slot": None,
        "description": (
            "包装材料选择依据的验证研究资料，包含稳定性研究、灭菌验证等（选填）"
        ),
        "sort_order": 6,
    },
]
...
S6_FIELD_MAPPINGS: list[dict[str, Any]] = [
    {
        "chapter_code": "3.2.S.6",
        "field_name": "包装形式",
        "field_type": "text",
        ...
        "extraction_prompt": (
            "从原料药质量标准文档中提取'包装形式'字段值，通常在'5 包装形式：...'"
            "段落中"
        ),
    ...
    },
    {
        ...
        "extraction_prompt": (
            "从原料药质量标准文档中提取'包装规格'字段值，通常在'6 规格：...'或"
            "'包装规格：...'段落中"
        ),
        "source_type": "asset_extract",
        "source_category": "原料药质量标准",
    ...
    },
    {
        ...
        "extraction_prompt": (
            "从包材质量标准文档中提取厂内使用的物料名称，通常在'物料代码'或"
            "'基本信息'段落中"
        ),
        "source_type": "asset_extract",
        "source_category": "包材质量标准",
    ...
    },
    {
        ...
        "extraction_prompt": (
            "从包材质量标准文档中提取完整的检验项目表格，包含序号、检验项目、"
            "企业内控标准等列，返回二维数组格式"
        ),
        "source_type": "asset_extract",
        "source_category": "包材质量标准",
...
async def seed() -> None:
    async with async_session_factory() as db:
        ...
        await db.commit()
        print(
            f"✓ S.6 种子数据写入完成: {len(S6_ASSET_CATEGORIES)} 个素材分类, "
            f"{len(S6_FIELD_MAPPINGS)} 个字段映射"
        )
