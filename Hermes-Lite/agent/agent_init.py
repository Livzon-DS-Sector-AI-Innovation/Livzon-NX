...
from hermes_cli._stubs import get_provider_request_timeout
...
def init_agent(
    ...
    try:
        from hermes_cli._stubs import set_current_session_id
    ...
    if not skip_memory:
        try:
            ...
            if agent._memory_enabled or agent._user_profile_enabled:
                # LeadFlow: per-user memory isolation via memory_service.
                # When user_id is present (web API / gateway), USER.md is
                # scoped to memories/users/{user_id}/USER.md. MEMORY.md
                # stays global (shared engine knowledge).
                try:
                    from services.memory_service import create_memory_store_for_user
                    agent._memory_store = create_memory_store_for_user(
                        user_id=getattr(agent, "_user_id", None),
                        memory_char_limit=mem_config.get("memory_char_limit", 2200),
                        user_char_limit=mem_config.get("user_char_limit", 1375),
                    )
                except ImportError:
                    # Fallback to direct MemoryStore if memory_service unavailable
        ...
        try:
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        from hermes_cli._stubs import get_active_profile_name
    ...
    # Resolve api_key from environment variables for providers that
    # don't have a hardcoded key in config.yaml.  Env var naming
    # convention: <PROVIDER_NAME_UPPER>_API_KEY  (e.g. XIAOMI_API_KEY).
    # This keeps secrets out of config.yaml and centralises them in .env.
    import os as _os
    for _cp_entry in _custom_providers:
        ...
        if _cp_key:
            continue  # config.yaml key takes precedence
        _cp_name = (_cp_entry.get("name") or "").strip().upper().replace("-", "_")
        if _cp_name:
            _env_key = f"{_cp_name}_API_KEY"
            _cp_entry["api_key"] = _os.getenv(_env_key, "")
