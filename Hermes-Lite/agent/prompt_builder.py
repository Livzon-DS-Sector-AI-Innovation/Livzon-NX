...
def _scan_context_content(content: str, filename: str) -> str:
    ...
    if findings:
        descriptions = [f.get("description", f.get("category", "unknown")) for f in findings]
        logger.warning("Context file %s blocked: %s", filename, ", ".join(descriptions))
        return f"[BLOCKED: {filename} contained potential prompt injection ({', '.join(descriptions)}). Content not loaded.]"
...
def build_skills_system_prompt(
    ...
    from hermes_cli._stubs import get_session_env
    _platform_hint = (
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    try:
        from hermes_cli._stubs import get_nous_subscription_features
