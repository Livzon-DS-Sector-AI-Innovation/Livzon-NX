...
from agent._stubs import _summarize_user_message_for_log
...
from agent._stubs import estimate_usage_cost, normalize_usage
...
try:
    ...
    _THREAT_DETECTION_AVAILABLE = True
except ImportError:
    _THREAT_DETECTION_AVAILABLE = False
...
def _try_refresh_nous_paid_entitlement_credentials(agent) -> bool:
    ...
    try:
        from hermes_cli._stubs import get_nous_portal_account_info
...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    try:
        from agent._stubs import seed_credits_at_session_start
...
def run_conversation(
    ...
    # ── Security: Prompt Injection Detection ─────────────────────────────
    # Scan user message for prompt injection attempts before processing.
    # This is a defense-in-depth measure to detect and log malicious inputs.
    if _THREAT_DETECTION_AVAILABLE and isinstance(user_message, str) and user_message.strip():
        try:
            threat_warning = first_threat_message(user_message)
            if threat_warning:
                # Log the threat for security monitoring
                logger.warning(
                    "SECURITY: Prompt injection detected in session=%s user_id=%s: %s",
                    agent.session_id or "none",
                    getattr(agent, 'user_id', 'unknown'),
                    threat_warning,
                )
                # Emit status callback for observability
                agent._emit_status(threat_warning)

                # Block threshold: critical always blocked; high blocked in
                # non-CLI contexts (web API / gateway) where there is no
                # interactive user to review and approve.
                threats = scan_for_threats(user_message, max_threats=1)
                if threats:
                    sev = threats[0].get("severity", "")
                    should_block = sev == "critical"
                    if not should_block and sev == "high":
                        # CLI interactive sessions allow high-severity with
                        # a warning (user is present to judge). Non-interactive
                        # contexts (web API, gateway bots) block to prevent
                        # prompt injection from executing unchecked.
                        is_interactive = env_var_enabled("HERMES_INTERACTIVE")
                        if not is_interactive:
                            should_block = True
                    if should_block:
                        block_reason = f"prompt_injection_{sev}"
                        logger.critical(
                            "SECURITY: %s prompt injection blocked in session=%s (reason=%s)",
                            sev.upper(), agent.session_id or "none", block_reason,
                        )
                        return {
                            "response": (
                                "⚠️ Security Alert: Your message contains patterns that match "
                                "known prompt injection attacks and has been blocked for safety. "
            ...
            # Don't let threat detection errors break the conversation
            logger.debug("Threat detection error (non-fatal): %s", e)
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if agent.provider == "nous":
                    try:
                        from agent._stubs import clear_nous_rate_limit
                ...
                if (
                    ...
                    from agent._stubs import is_token_provider
