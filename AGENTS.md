...
用户要求提交或发布时，先核对范围；已有明确范围时由代理核对并继续，不重复询问。仅在范围不清、存在无关改动且无法安全隔离时询问，不得混入无关修改或覆盖用户工作。
...
仅本地提交不执行 fetch、分支同步、推送或合并；执行步骤 1 的范围核对、步骤 4 的适用本地验证及提交内容复核即可。完整流程中，步骤 2–4 连续执行且没有长时间开发或等待时可复用同次成功 fetch；步骤 5 推送前仍须重新 fetch 并核对。
...
2. **保护工作并准备分支**：工作区干净且没有待保护的本次提交时，可先切换到
   ...
   覆盖文件。
3. **同步开发分支基线**：保护工作后，在工作区安全的前提下执行或按上述条件复用
...
4. **提交前同步与本地验证**：完整流程准备提交前执行或按上述条件复用
   `git fetch origin main --prune`；仅本地提交跳过远端同步。如果 `origin/main` 自上次同步后前进，完整流程必须
   ...
   涉及后端迁移、数据库模型或需要启动本地开发后端时，必须先确认目标是本次授权的
   本地开发库，并使用根目录 `.env.local` 和 `compose.dev.yml` 检查 Alembic 状态：
   `heads` 必须只有一个，且应比较数据库 `current` 与代码 `head`。数据库落后时，
   先更新开发后端镜像所需依赖，再通过开发栈的 `migrate` 服务执行
   `alembic upgrade head`；根目录命令为
   `docker compose --env-file .env.local -f compose.dev.yml run --rm migrate`。禁止用
   `stamp`、手工 SQL 或修改
   `alembic_version` 跳过迁移。迁移失败、出现多个 head、版本不在当前迁移链或目标
   数据库无法确认时必须停止，不得继续启动后端。迁移成功后再次确认
   `current == head`，再重建或重启后端 `app` 容器，等待容器健康检查通过并验证
   `/health` 返回成功；迁移和健康结果写入本次验证记录。共享库、生产库及破坏性
   迁移仍按数据库安全规则另行取得明确授权。
   完整推送合并流程中，本地先运行覆盖改动及受影响调用链的定向测试和必要契约
   检查；影响较广时扩大相关检查范围，不把本地全量测试作为推送前的默认门槛。
   全量测试和覆盖率门禁交由最新 PR head 的 CI 执行；仅在子项目规范明确要求
   本地全量验证、需复现 CI 失败或验证 CI 无法覆盖的本地风险时运行本地全量测试。
   如果本次启动的本地全量测试与 CI 重复且持续拖慢流程，可安全停止该进程，记录
   未完成范围，完成必要定向验证后推进；不得将中止的测试记为通过。主线同步后
   根据新增差异及冲突解决内容重跑受影响检查；不得复用同步前的 CI 结果作为
   合并依据。
...
9. **合并后收口同步**：确认 PR 已合并及目标提交后，执行
   ...
   擅自重写或删除该分支。主线同步后若本地开发后端或数据库正在使用，必须再次按
   步骤 4 检查数据库 `current` 与最新代码唯一 `head`；数据库落后时完成迁移，随后
   重建或重启后端并验证容器健康和 `/health`，避免合并后继续运行旧 schema。

## 验证与交付

验证范围必须与修改风险匹配，并执行受影响子项目 `AGENTS.md` 规定的检查。跨项目修改分别验证各项目；无法执行的检查必须说明原因和风险，不得伪造结果。

耗时检查应先确认依赖关系，再让互不依赖的本地测试、静态检查、构建及跨项目验证并行执行；推送合并流程中的完整集成测试也应与无产物依赖的质量、安全和镜像检查并行。迁移必须先于依赖该数据库状态的测试，生成产物必须先于消费它的检查。共享测试库、输出目录或资源不足时使用隔离环境或限制并发；收集并核对所有必需检查的结果，不因并行执行跳过失败项或缩减门禁。
